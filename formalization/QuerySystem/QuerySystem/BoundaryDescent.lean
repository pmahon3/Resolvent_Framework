...
/-- Two independent Boolean coordinates can be repaired simultaneously on
finite traces.  No compatibility equation between the two chosen carrier
...
theorem independent_finite_traces_dirac (hMeets : MeetsExist d)
    ...
    ∃ ω η : Ω,
      (∀ A ∈ s, (ω ∈ A ↔ μ.Val A)) ∧
      (∀ A ∈ t, (η ∈ A ↔ μ.Val A)) := by
  obtain ⟨ω, hω⟩ := finite_trace_dirac hMeets μ hM hs hsubM
  obtain ⟨η, hη⟩ := finite_trace_dirac hMeets μ hN ht hsubN
  exact ⟨ω, η, hω, hη⟩
...
#print axioms independent_finite_traces_dirac

...
noncomputable def cylGen_addContent
  ...
  empty' := by
    -- ∅ = Cyl i ∅ for any i, and cylGenMass ... (Cyl i ∅) _ = ν i ∅ = 0.
    haveI : Decidable (∅ ∈ S.CylGen) := Classical.propDecidable _
    obtain ⟨i⟩ := ‹Nonempty S.ι›
    have hempty_eq : (∅ : Set S.Omega) = S.Cyl i ∅ := by ext ω; simp [Cyl, eval]
    have hmem : (∅ : Set S.Omega) ∈ S.CylGen :=
      ⟨i, ∅, MeasurableSet.empty, hempty_eq⟩
    have hmem₂ : S.Cyl i ∅ ∈ S.CylGen := ⟨i, ∅, MeasurableSet.empty, rfl⟩
    simp only [hmem, dite_true]
    -- cylGenMass ... ∅ hmem = 0: use cylGenMass_eq_marginal after rewriting ∅ = Cyl i ∅
    -- The set ∅ equals Cyl i ∅ by hempty_eq; rewrite in the cylGenMass first argument.
    -- cylGenMass is about a set E and a proof hE : E ∈ CylGen.
    -- We have hmem : ∅ ∈ CylGen. Rewrite the set ∅ to Cyl i ∅ to get hmem₂.
    -- To transport cylGenMass, use hempty_eq to Eq.mpr the proof:
    have hmem_cast : S.Cyl i ∅ ∈ S.CylGen := hempty_eq ▸ hmem
    -- cylGenMass ... ∅ hmem = cylGenMass ... (Cyl i ∅) hmem_cast  (by cylGenMass_wellDef, same set!)
    -- Wait: ∅ = Cyl i ∅ but cylGenMass_wellDef needs SAME set. ∅ and Cyl i ∅ differ only propositionally.
    -- Use: hempty_eq to rewrite the set argument in the goal.
    -- Goal: cylGenMass ... ∅ hmem = 0
    -- Rewrite ∅ → Cyl i ∅ in the goal using hempty_eq:
    conv_lhs => rw [hempty_eq]
    -- Now goal: cylGenMass ... (Cyl i ∅) (_ : Cyl i ∅ ∈ CylGen) = 0
    rw [S.cylGenMass_wellDef udir surj ν compat (S.Cyl i ∅) _ hmem₂]
    rw [S.cylGenMass_eq_marginal udir surj ν compat i ∅ MeasurableSet.empty]
    simp
  sUnion' := by
    -- Finite additivity: for pairwise-disjoint I ⊆ CylGen with ⋃₀ I ∈ CylGen,
    -- toFun (⋃₀ I) = ∑ u ∈ I, toFun u.
    -- Strategy: induction on I using Finset.induction; binary step via preμ_disjoint_union.
    intro I hI_ss hI_dis hI_mem
    haveI : DecidableEq (Set S.Omega) := Classical.decEq _
    induction I using Finset.induction with
    | empty =>
      -- ⋃₀ ∅ = ∅; toFun ∅ = 0 since cylGenMass on ∅ = ν i ∅ = 0
      simp only [Finset.coe_empty, Set.sUnion_empty, Finset.sum_empty]
      simp only [hI_mem, dite_true, cylGenMass]
      -- same argument as empty': the choose presentation (i', A') satisfies A' = ∅
      haveI : DecidableEq S.ι := Classical.decEq S.ι
      set i' := hI_mem.choose
      set A' := hI_mem.choose_spec.choose
      have hA'meas : MeasurableSet A' := hI_mem.choose_spec.choose_spec.1
      have hA'empty : (∅ : Set S.Omega) = S.Cyl i' A' := by
        have := hI_mem.choose_spec.choose_spec.2
        simp only [Finset.coe_empty, Set.sUnion_empty] at this
        exact this
      have hA'_eq_empty : A' = ∅ := by
        have : S.eval i' ⁻¹' A' = ∅ := by change S.Cyl i' A' = ∅; exact hA'empty.symm
        exact Set.preimage_injective.mpr (surj i') this
      rw [S.preμ_eq udir ν compat {i'} _
        (fun j hj => by have := Finset.mem_singleton.mp hj; subst this; simpa [dif_pos rfl])
        (fun j hj => by have := Finset.mem_singleton.mp hj; subst this; exact S.le_refl i')]
      rw [S.preμAt_single_eq_marginal ν i' _]
      simp only [dif_pos rfl]
      rw [hA'_eq_empty]; simp
    | insert ha ih =>
      -- ha : a ∉ s; ih is the induction hypothesis for s
      -- The names introduced by Finset.induction are (a : Set S.Omega) and (s : Finset (Set S.Omega))
      rename_i a₀ s₀
      -- Introduce local abbreviations for readability
      set E := a₀
      set I' := s₀
      have hE_mem : E ∈ S.CylGen := hI_ss (Finset.mem_insert_self E I')
      have hI'_ss : ↑I' ⊆ S.CylGen := fun x hx => hI_ss (Finset.mem_insert_of_mem hx)
      have hI'_dis : PairwiseDisjoint (I' : Set (Set S.Omega)) id :=
        hI_dis.subset (Finset.coe_subset.mpr (Finset.subset_insert E I'))
      -- ⋃₀ insert E I' = E ∪ ⋃₀ I'
      have hUnion_eq : ⋃₀ ↑(Finset.insert E I') = E ∪ ⋃₀ ↑I' := by
        simp [Finset.coe_insert, Set.sUnion_insert]
      -- ⋃₀ I' ∈ CylGen (needed for ih and for the binary union)
      -- The union E ∪ ⋃₀ I' ∈ CylGen (from hI_mem + hUnion_eq)
      -- E and ⋃₀ I' are disjoint (E ∉ I', pairwise disjoint)
      have hDisj_E_rest : Disjoint E (⋃₀ ↑I') := by
        rw [Set.disjoint_sUnion_right]
        intro F hF
        apply hI_dis (Finset.mem_insert_self E I')
          (Finset.mem_insert_of_mem hF)
        intro h; simp at ha; exact ha (h ▸ hF)
      -- ⋃₀ ↑I' ∈ CylGen: needed for ih
      -- Build a MeasFinCyl U for E ∪ ⋃₀ I'
      obtain ⟨i_U, A_U, hA_U, hU_eq⟩ := hI_mem
      obtain ⟨i_E, A_E, hA_E, hE_eq⟩ := hE_mem
      -- ⋃₀ I' = (⋃₀ ↑(insert E I')) \ E = (Cyl i_U A_U) \ (Cyl i_E A_E)
      -- but it's easier to just note that ⋃₀ I' ∈ CylGen from the difference
      -- Actually, compute: ⋃₀ I' = (E ∪ ⋃₀ I') \ E = (⋃₀ insert E I') \ E
      have hrest_eq : ⋃₀ ↑I' = ⋃₀ ↑(Finset.insert E I') \ E := by
        rw [hUnion_eq]; exact (Set.union_diff_cancel_left hDisj_E_rest.symm.le_bot.eq_bot ▸
          by rw [Set.union_diff_left, Set.diff_eq_empty.mpr]; exact Set.disjoint_left.mp hDisj_E_rest).symm
        all_goals rfl
      -- ⋃₀ I' ∈ CylGen via isSetSemiring_CylGen.diff_eq_sUnion'
      have hSR : IsSetSemiring S.CylGen := S.isSetSemiring_CylGen udir
      -- The content of ⋃₀ ↑I' is in CylGen (from semiring difference closure):
      -- ⋃₀ ↑I' = (⋃₀ insert E I') \ E, both are in CylGen
      -- Use IsSetSemiring.diff_eq_sUnion' to get ⋃₀ I' ∈ CylGen
      have hrest_mem : ⋃₀ ↑I' ∈ S.CylGen := by
        -- (⋃₀ insert E I') \ E ∈ CylGen via IsSetSemiring, since both are in CylGen
        -- More directly: disjoint union → ⋃₀ I' = total \ E
        -- Use: ⋃₀ I' = ⋃₀ (insert E I') \ E
        -- Both ⋃₀ (insert E I') and E are in CylGen; difference is a finite union in CylGen
        rcases hSR.diff_eq_sUnion' hI_mem hE_mem with ⟨J, hJ_ss, hJ_dis, hJ_mem, hJ_eq⟩
        -- J is a finite disjoint collection in CylGen with ⋃₀ J = hI_mem.1 \ E
        -- But we need ⋃₀ I' ∈ CylGen
        -- Since ⋃₀ insert E I' = E ∪ ⋃₀ I' and E ∩ ⋃₀ I' = ∅,
        -- ⋃₀ I' = (⋃₀ insert E I') \ E which equals ⋃₀ J ∈ CylGen (finite union)
        -- ⋃₀ J ∈ CylGen since J is a finite subset of CylGen (π-system closed under intersections,
        -- and finite disjoint unions...)
        -- Actually ⋃₀ J ∈ CylGen only if J has one element; in general CylGen is not closed under unions.
        -- But hJ_eq says ⋃₀ ↑I' = hI_mem \ E = ⋃₀ J, and hJ_mem says ⋃₀ J ∈ CylGen.
        rw [← hUnion_eq] at hJ_eq ⊢
        rw [show (⋃₀ ↑(Finset.insert E I') \ E) = ⋃₀ ↑I' from by
          rw [hUnion_eq]; simp [Set.union_diff_left,
            Set.diff_eq_empty.mpr (Set.disjoint_left.mp hDisj_E_rest)]]
        rw [← hJ_eq]
        exact hJ_mem
      -- Now apply ih to I'
      have hih : (fun E_1 =>
        haveI : Decidable (E_1 ∈ S.CylGen) := Classical.propDecidable _
        if h : E_1 ∈ S.CylGen then S.cylGenMass udir surj ν compat E_1 h else 0) (⋃₀ ↑I') =
        ∑ u ∈ I', (fun E_1 =>
        haveI : Decidable (E_1 ∈ S.CylGen) := Classical.propDecidable _
        if h : E_1 ∈ S.CylGen then S.cylGenMass udir surj ν compat E_1 h else 0) u :=
        ih hI'_ss hI'_dis hrest_mem
      -- Goal: toFun (E ∪ ⋃₀ I') = toFun E + ∑ u ∈ I', toFun u
      rw [← hUnion_eq]
      simp only [hI_mem, dite_true]
      -- toFun (⋃₀ insert E I') = cylGenMass ... (⋃₀ insert E I') hI_mem
      -- = ν i_U A_U   (by cylGenMass_eq_marginal via hU_eq)
      -- toFun E = cylGenMass ... E hE_mem = ν i_E A_E
      -- ∑ u ∈ I', toFun u = (toFun (⋃₀ I') from ih)
      -- Use preμ_disjoint_union to get ν i_U A_U = ν i_E A_E + ν m (⋃₀ I' at m)
      -- This requires packaging as MeasFinCyl and calling preμ_disjoint_union
      -- Here we use cylGenMass_eq_marginal on both sides
      rw [S.cylGenMass_wellDef udir surj ν compat _ _ ⟨i_U, A_U, hA_U, hU_eq⟩]
      rw [show ⋃₀ ↑(Finset.insert E I') = S.Cyl i_U A_U from hU_eq.symm]
      rw [S.cylGenMass_eq_marginal udir surj ν compat i_U A_U hA_U]
      -- Sum: ∑ u ∈ insert E I', toFun u = toFun E + ∑ u ∈ I', toFun u
      rw [Finset.sum_insert ha]
      simp only [hE_mem, dite_true]
      rw [S.cylGenMass_wellDef udir surj ν compat E _ hE_mem]
      rw [show E = S.Cyl i_E A_E from hE_eq.symm]
      rw [S.cylGenMass_eq_marginal udir surj ν compat i_E A_E hA_E]
      -- Now goal: ν i_U A_U = ν i_E A_E + ∑ u ∈ I', toFun u
      -- hih says: toFun (⋃₀ I') = ∑ u ∈ I', toFun u
      -- ⋃₀ I' ∈ CylGen, so toFun (⋃₀ I') = cylGenMass ... (⋃₀ I') hrest_mem
      simp only [hrest_mem, dite_true] at hih
      -- Get the index/set for ⋃₀ I'
      obtain ⟨i_R, A_R, hA_R, hR_eq⟩ := hrest_mem
      rw [S.cylGenMass_wellDef udir surj ν compat _ _ ⟨i_R, A_R, hA_R, hR_eq⟩] at hih
      rw [show ⋃₀ ↑I' = S.Cyl i_R A_R from hR_eq.symm] at hih
      rw [S.cylGenMass_eq_marginal udir surj ν compat i_R A_R hA_R] at hih
      rw [← hih]
      -- Now goal: ν i_U A_U = ν i_E A_E + ν i_R A_R
      -- Use preμ_disjoint_union with presentations:
      -- U = total union, C = E part, D = rest part
      let C_mfc : S.MeasFinCyl := ⟨{i_E}, fun j => if h : j = i_E then h ▸ A_E else ∅,
        fun j hj => by simp [Finset.mem_singleton.mp hj, hA_E]⟩
      let D_mfc : S.MeasFinCyl := ⟨{i_R}, fun j => if h : j = i_R then h ▸ A_R else ∅,
        fun j hj => by simp [Finset.mem_singleton.mp hj, hA_R]⟩
      let U_mfc : S.MeasFinCyl := ⟨{i_U}, fun j => if h : j = i_U then h ▸ A_U else ∅,
        fun j hj => by simp [Finset.mem_singleton.mp hj, hA_U]⟩
      have hCset : C_mfc.set = S.Cyl i_E A_E := S.finCyl_singleton_eq_cyl i_E A_E
      have hDset : D_mfc.set = S.Cyl i_R A_R := S.finCyl_singleton_eq_cyl i_R A_R
      have hUset : U_mfc.set = S.Cyl i_U A_U := S.finCyl_singleton_eq_cyl i_U A_U
      have hDisj_mfc : Disjoint C_mfc.set D_mfc.set := by
        rw [hCset, hDset, ← hE_eq.symm, ← hR_eq.symm]
        exact hDisj_E_rest
      have hUnion_mfc : U_mfc.set = C_mfc.set ∪ D_mfc.set := by
        rw [hUset, hCset, hDset, ← hE_eq.symm, ← hR_eq.symm, ← hU_eq.symm]
        rw [← hUnion_eq]
      have := S.preμ_disjoint_union udir ν compat surj C_mfc D_mfc U_mfc hDisj_mfc hUnion_mfc
      -- this : preμ ... U = preμ ... C + preμ ... D
      rw [S.preμ_eq udir ν compat {i_U} U_mfc.A U_mfc.hA
          (fun j hj => Finset.mem_singleton.mp hj ▸ S.le_refl j),
        S.preμ_eq udir ν compat {i_E} C_mfc.A C_mfc.hA
          (fun j hj => Finset.mem_singleton.mp hj ▸ S.le_refl j),
        S.preμ_eq udir ν compat {i_R} D_mfc.A D_mfc.hA
          (fun j hj => Finset.mem_singleton.mp hj ▸ S.le_refl j)] at this
      rw [S.preμAt_single_eq_marginal ν i_U U_mfc.A,
          S.preμAt_single_eq_marginal ν i_E C_mfc.A,
          S.preμAt_single_eq_marginal ν i_R D_mfc.A] at this
      convert this using 2 <;> simp [C_mfc, D_mfc, U_mfc]

...
lemma cylGenMass_eq_marginal
  ...
  -- cylGenMass unfolds to preμ udir ν {i} A' where A' j = if h : j = i then h ▸ A else ∅
  simp only [cylGenMass]
  -- The Classical.choose of ⟨i, A, hA, rfl⟩ is i, and its spec gives A
  -- Rewrite preμ as preμAt at i (i is the upper bound of {i})
  let A' : ∀ j : S.ι, Set ((S.q j).Outcome) := fun j => if h : j = i then h ▸ A else ∅
  have hA' : ∀ j ∈ ({i} : Finset S.ι), MeasurableSet (A' j) := by
    intro j hj
    simp only [Finset.mem_singleton] at hj
    subst hj
    simp only [A', dif_pos rfl]
    exact hA
  have hk : ∀ j ∈ ({i} : Finset S.ι), S.le j i :=
    fun j hj => Finset.mem_singleton.mp hj ▸ S.le_refl j
  rw [S.preμ_eq udir ν compat {i} A' hA' hk]
  rw [S.preμAt_single_eq_marginal ν i A']
  simp only [A', dif_pos rfl]
...
noncomputable def cylGen_addContent
  ...
  sUnion' := by
    ...
    -- Each element of I is in CylGen; the union is in CylGen
    -- Use induction on I
    induction I using Finset.induction_on with
    | empty => simp
    | insert ht ih =>
        rename_i t I' hnotmem
        -- Split the sUnion
        ...
        have hI'_mem : ⋃₀ ↑I' ∈ S.CylGen := by
          ...
          · simp [hne]; obtain ⟨i⟩ := ‹Nonempty S.ι›
            exact ⟨i, ∅, MeasurableSet.empty, by ext ω; simp [Cyl, eval]⟩
          · -- ⋃₀ ↑I' ∈ CylGen follows from hI_mem by noting t ∪ ⋃₀ I' = ⋃₀ (insert t I')
            -- and using isSetSemiring (closed under binary union for CylGen elements)
            -- Actually we derive it: ⋃₀ (insert t I') ∈ CylGen, so by semiring closure
            -- just note the union is the full sUnion minus t... this is hard in general.
            -- Instead use: t and ⋃₀ I' are disjoint, their union is ⋃₀ (insert t I') ∈ CylGen.
            -- The simplest route: use isPiSystem / isSetSemiring to get closure.
            -- CylGen is closed under disjoint binary union (it's a SetSemiring).
            -- t ∈ CylGen; t ∪ ⋃₀ I' = ⋃₀ ↑(insert t I') = hI_mem
            -- So ⋃₀ I' = (⋃₀ ↑(insert t I')) \ t ∈ CylGen by diff_mem of SetSemiring? No, that's not right.
            -- Best approach: ⋃₀ ↑(insert t I') ∈ CylGen and t ∈ CylGen; disjoint;
            -- hence each component is in CylGen (since CylGen is a SetSemiring with disjoint decomp).
            -- Actually the simplest fact: t and ⋃₀ I' are disjoint subsets of ⋃₀ (insert t I') ∈ CylGen
            -- and t ∈ CylGen. By isSetSemiring_CylGen, (⋃₀ (insert t I')) \ t ∈ SetSemiring.
            -- Since they're disjoint, ⋃₀ I' = (⋃₀ (insert t I')) \ t.
        ...
        -- Use preμ_disjoint_union to split mass of (t ∪ ⋃₀ I')
        ...
        -- cylGenMass is preμ on chosen presentations
        -- Use preμ_disjoint_union via MeasFinCyl packaging
        rcases ht_mem with ⟨it, At, hAt, rfl⟩
        rcases hI'_mem with ⟨iu, Au, hAu, rfl⟩
        rcases hI_mem with ⟨iU, AU, hAU, hUeq⟩
        -- Package as MeasFinCyl
        ...
        have hCU_set : CU.set = S.Cyl iU AU := S.finCyl_singleton_eq_cyl iU AU
        -- mass via cylGenMass_eq_marginal
        ...
        have hmU : S.cylGenMass udir surj ν compat (S.Cyl iU AU) ⟨iU, AU, hAU, rfl⟩ =
            ν iU AU := S.cylGenMass_eq_marginal udir surj ν compat iU AU hAU
        -- The union equation: S.Cyl iU AU = S.Cyl it At ∪ ⋃₀ ↑I'
        -- but ⋃₀ ↑I' = S.Cyl iu Au (from hI'_mem decomposition)... not necessarily!
        -- hI'_mem says ⋃₀ I' ∈ CylGen; we destructured it as Cyl iu Au.
        -- The full union hUeq says S.Cyl iU AU = S.Cyl it At ∪ ⋃₀ ↑I'
        -- = S.Cyl it At ∪ S.Cyl iu Au.
        -- Apply preμ_disjoint_union
        have hUnionCD : CU.set = Ct.set ∪ Cu.set := by
          rw [hCt_set, hCu_set, hCU_set]
          -- hUeq : S.Cyl iU AU = S.Cyl it At ∪ ⋃₀ ↑I'
          -- and ⋃₀ I' = S.Cyl iu Au (from our destructuring of hI'_mem)
          -- We need to verify ⋃₀ I' = Cu.set = Cyl iu Au
          -- This follows from ih' which told us toFun (⋃₀ I') = ∑ I' and hI'_mem
          -- Actually we know ⋃₀ ↑I' = Cyl iu Au from hI'_mem destructuring? No — hI'_mem
          -- was ⟨iu, Au, hAu, rfl⟩ which says Cyl iu Au ∈ CylGen and we set ⋃₀ I' = Cyl iu Au.
          -- Wait: we rcased hI'_mem as ⟨iu, Au, hAu, rfl⟩, meaning ⋃₀ ↑I' = Cyl iu Au.
          -- Similarly hI_mem as ⟨iU, AU, hAU, hUeq⟩ means Cyl iU AU = hUeq is the eq to full.
          -- Actually the rcases gives hUeq : S.Cyl iU AU = ... no, rcases on ∃ gives the witness.
          -- Let me re-examine: hI_mem : (t ∪ ⋃₀ ↑I') ∈ S.CylGen, rcased as ⟨iU, AU, hAU, hUeq⟩
          -- where hUeq : S.Cyl iU AU = t ∪ ⋃₀ ↑I'? No — CylGen = {Cyl i A} so
          -- hUeq : t ∪ ⋃₀ ↑I' = S.Cyl iU AU.
          rw [← hUeq]
          -- and ⋃₀ ↑I' was rcased from hI'_mem as = Cyl iu Au
          -- but we set ⋃₀ I' to be the thing and rcased hI'_mem ⟨iu,Au,hAu,rfl⟩
          -- rfl means ⋃₀ ↑I' = Cyl iu Au, so the goal S.Cyl it At ∪ Cyl iu Au = t ∪ Cyl iu Au
          -- t = Cyl it At (from rcases ht_mem ⟨it,At,hAt,rfl⟩)
          rfl
        ...
        -- Now connect cylGenMass to preμ via wellDef
        simp only [Finset.sum_insert hnotmem]
        rw [hval_full, hval_t]
        -- ih' connects toFun (⋃₀ I') to ∑ I'
        -- toFun (⋃₀ I') = cylGenMass ... (⋃₀ I') hI'_mem
        -- and ⋃₀ I' = Cyl iu Au from rfl destructuring
        -- So ih' says dif_pos hI'_mem version = ∑ u ∈ I', toFun u
        rw [← ih']
        -- Now: cylGenMass(Cyl iU AU) = cylGenMass(Cyl it At) + toFun(⋃₀ I')
        -- cylGenMass uses preμ; connect via preμ_disjoint_union
        ...
        rw [hmu]
        -- Now need ν iU AU = ν it At + ν iu Au
        -- This follows from preμ_disjoint_union: preμ(CU) = preμ(Ct) + preμ(Cu)
        -- preμ of singleton = ν i A (via preμ_eq + preμAt_single_eq_marginal)

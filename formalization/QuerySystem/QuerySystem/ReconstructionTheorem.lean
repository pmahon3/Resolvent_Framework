...
/-- The delay map `Φ_h : X → (ℤ → ℝ)`, `Φ_h(x)(n) = h(T^[n.toNat] x)`. -/
def delayMap (h : X → ℝ) (T : X → X) : X → (ℤ → ℝ) :=
  fun x n => h (T^[n.toNat] x)
...
theorem delayMap_measurable
  ...
  intro n
  exact hh.comp (hT.iterate n.toNat)

/-- **Pullback identity (Lemma 2.3, Paper III):**
    `observableAlgebra (h ∘ T^[·]) = Φ_h⁻¹(ℬ(ℤ → ℝ))`.

    **Proof sketch:**
    - (≤): Each generator `h ∘ T^[n.toNat] = πₙ ∘ Φ_h`, so its level sets are
      in `Φ_h⁻¹(ℬ(ℤ → ℝ))`.
    - (≥): The product σ-algebra `ℬ(ℤ → ℝ)` is generated by coordinate projections
      `πₙ`, and `Φ_h⁻¹(πₙ⁻¹(E)) = (h ∘ T^[n.toNat])⁻¹(E) ∈ 𝒪_h`. -/
theorem observableAlgebra_eq_comap
    (h : X → ℝ) (T : X → X) (hh : Measurable h) (hT : Measurable T) :
    observableAlgebra (fun n => h ∘ T^[n.toNat]) =
    MeasurableSpace.comap (delayMap h T)
        (MeasurableSpace.pi (m := fun (_ : ℤ) => inferInstance)) := by
  sorry
  -- Both directions via observableAlgebra_le and MeasurableSpace.comap_le_iff_le_map

-- ============================================================
-- §5  Shift intertwining
-- ============================================================

/-- The bilateral shift `σ` on `ℤ → ℝ`: `(σ f)(n) = f(n + 1)`. -/
def bilateralShift : (ℤ → ℝ) → (ℤ → ℝ) := fun f n => f (n + 1)

/-- **Shift intertwining (Corollary 4.3, Paper III):** `Φ_h(T x) = σ(Φ_h(x))`.

    **Proof sketch:** For `n ≥ 0`, `(n + 1).toNat = n.toNat + 1` by
    `Int.toNat_add_one` and `T^[k+1] y = T^[k] (T y)` by `iterate_succ_apply`.
    For `n < 0`, both sides equal `h x` since `n.toNat = (n+1).toNat = 0`. -/
theorem delayMap_intertwines_shift (h : X → ℝ) (T : X → X) (x : X) :
    delayMap h T (T x) = bilateralShift (delayMap h T x) := by
  ext n
  simp only [delayMap, bilateralShift]
  congr 1
  sorry -- Int.toNat_add_one (for n ≥ 0) + Function.iterate_succ_apply
...
theorem cyclic_implies_dense
    ...
    (hmem : ∀ n : ℤ, MemLp (h ∘ T^[n.toNat]) 2 μ)
    (h_cyclic :
      (Submodule.span ℝ
        (Set.range (fun n : ℤ => (hmem n).toLp _))).topologicalClosure = ⊤) :
    Dense (lpMeas ℝ ℝ (observableAlgebra (fun n => h ∘ T^[n.toNat])) 2 μ :
      Set (Lp ℝ 2 μ)) := by
  sorry

...
def ConcreteSigmaOrtho {Ω : Type*} {d : DynkinSystem Ω}
    (s₀ : TwoValuedState d) (B : Block d) : Prop :=
  AbstractSigmaOrtho d ∧ WitnessAt s₀ B
...
theorem concreteSigmaOrtho_iff_witness
    {Ω : Type*} {d : DynkinSystem Ω} (s₀ : TwoValuedState d) (B : Block d) :
    ConcreteSigmaOrtho s₀ B ↔ WitnessAt s₀ B :=
...
theorem sigma_ortho_gap_is_wallA
    {Ω : Type*} {d : DynkinSystem Ω} (s₀ : TwoValuedState d) (B : Block d)
    (hi : kernel s₀ B = ∅) :
    ConcreteSigmaOrtho s₀ B ↔ WallA s₀ B := by
  rw [concreteSigmaOrtho_iff_witness]
  exact wallA_is_bottleneck s₀ B hi
...
theorem concreteSigmaOrtho_not_intersection_closed
    {Ω : Type*} {d : DynkinSystem Ω} (s₀ : TwoValuedState d) (B : Block d)
    (h : ConcreteSigmaOrtho s₀ B) :
    ¬ IntersectionClosed s₀ B :=
  witness_not_intersection_closed s₀ B h.2
...
theorem psi_iff_concreteSigma : Psi ↔ Psi_concreteSigma := by
  ...
  · rintro ⟨Ω, d, s₀, B, h⟩
    exact ⟨Ω, d, s₀, B, (concreteSigmaOrtho_iff_witness s₀ B).mp h⟩

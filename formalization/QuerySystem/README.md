...
17 pages including entropy characterisation) is mathematically complete in
LaTeX (`papers/paper_iv/`) but not yet formalized in Lean.
...
| `PredictiveState.lean` | II | Predictive kernel, minimal predictive state map, factorization | ✅ 0 sorrys |
...
| `lpMeas_eq_top_of_ae_eq` | `ReconstructionTheorem.lean` | No downward `AEStronglyMeasurable` monotonicity in Mathlib |
| `reconstruction_iff_lpMeas` (←) | `ReconstructionTheorem.lean` | `tendsto_ae_of_tendsto_Lp` not in Mathlib |

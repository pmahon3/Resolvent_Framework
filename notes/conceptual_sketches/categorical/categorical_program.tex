...
realizable), the marginalisation functor $\Phi$ is an \emph{isomorphism of
categories}: every compatible family $\{\nu_i\}$ extends uniquely to a measure
$P$ on $\Omega$, and $\Phi(P) = \{\nu_i\}$. So $\Phi$ has a two-sided inverse
(the extension functor) and the adjunction $\mathrm{Ext} \dashv \Phi$ is in fact
an equivalence.
\end{theorem}

\noindent The careful development --- the categories $\mathrm{QSys}$,
$\mathrm{PSys}$, the functors, the unit/counit, and the proof (uniqueness =
Paper~1 Determination Thm; existence = the Carathéodory route, realizability
replacing tightness; Lean: \texttt{observational\_extension} in
\texttt{QuerySystem.lean}) --- is in the companion
\texttt{categorical\_adjunction.tex} (Thms.~on the adjunction and the
equivalence). It is not repeated here.
...
See \texttt{mathlib\_gaps.md} (this directory) for the consolidated audit
(Kleisli / Eilenberg--Moore / Giry monad / Markov kernels present; projective
limits and Kolmogorov extension absent; \texttt{QuerySystem.lean}'s
\texttt{observational\_extension} fills the extension gap in-house for the query
setting). The one gap specific to \emph{this} document: the adjunction of
Theorem~\ref{thm:extension-adjunction} (defining $\Prob(\Omega)$ and
$\mathrm{Compat}(\{O_i\})$ as Lean~4 categories and verifying the adjunction
axioms) is not formalized --- the next categorical formalization target.

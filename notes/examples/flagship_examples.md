...
At lag $L$, the delay algebra captures the $\sigma$-algebra generated by
ternary digits $0$ through $L$. The error $\delta(L)$ decays at rate
$(2/3)^L$ (determined by the Cantor measure's self-similarity), with no
smoothness index required.
...
- [ ] Make the ternary digit argument precise (measurability of the
  separation events)
- [ ] Compute $\delta(L)$ explicitly and verify the $(2/3)^L$ decay rate

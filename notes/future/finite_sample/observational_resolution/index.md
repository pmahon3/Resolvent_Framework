...
### Relation to Young's formula

Young's dimension formula is the classical smooth measure-relative benchmark
for this pattern.  For a $C^2$ surface diffeomorphism with ergodic invariant
measure $\mu$ and Lyapunov exponents

$$
\lambda_1>0>\lambda_2,
$$

Young proves, in this setting,
...
full-$\mu$-measure sets.

In $D_\Lambda$ language this is not prior to measure.  It is a downstream
smooth-dynamical specialization:

- the measure $\mu$ is already given;
- the numerator is measure entropy $h_\mu(f)$, the typical growth rate of
  orbit distinctions;
- the valuation is supplied by the Lyapunov exponents, which convert orbit time
  into metric scale;
- the refinements are metric/Bowen refinements generated by the smooth
  dynamics.

Heuristically, a ball of radius $r$ requires about
$\log(1/r)/\lambda_1$ forward iterates to resolve the unstable direction and
$\log(1/r)/|\lambda_2|$ backward iterates to resolve the stable direction.
Entropy then gives
...
so the local dimension is exactly the entropy growth divided by the
Lyapunov-supplied valuation.

The programme-level distinction is important: $D_\Lambda$ is the abstract
valued-refinement schema.  If $H$ is atom-count entropy it can be prior to a
measure; if $H$ is Shannon, collision, or metric entropy of a law, it is
measure-relative.  Young lives in the measure-relative branch, after Paper I
has supplied probability and after Paper II has supplied smooth reconstructed
dynamics.

...
($O_B$ open), so $\le\aleph_0$ atomic blocks force $S_{\mathrm{df}}^\sigma$ to be
$G_\delta$ and hence \emph{no witness} (Exit~B); a witness \emph{necessarily} needs
uncountably many infinite atomic blocks, leaving one live construction target
(uncountably many atomic blocks on a countable ground set, $|L|=\mathfrak c$). For

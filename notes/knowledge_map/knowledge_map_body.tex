...
closure step is state-free), so the class is \textbf{inhabited as
stated} and the ``richness starves concreteness'' conjecture is
\textbf{false as literally worded}.  The whole question reduces to
one orthocomplement check: in the substituted logic $\mathcal{L}_2$,
is $a\,|\,C \not\perp b\,|\,C$ for distinct non-complementary
$\mathrm{MO}_2$ atoms?  A candidate witness
$p=\bigvee_n b\,|\,C_n$ satisfies the interleaving condition in the
ambient product; only its transfer to $\mathcal{L}_2$ is open.  If
the check holds, $\mathcal{L}_2$ is the sought point-free
relational-probability object; if not, it segregates and $L(H)$
remains the only known witness.  The by-hand computation is the
open step; see
\texttt{open\_questions/verification/inhabitation\_check.md}.

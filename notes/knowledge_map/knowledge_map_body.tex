...
open.  Likely ZFC-independent but this is not proved.
...
\paragraph{Descent axis sharpened (2026-06-08).}
...
$\sigma$-additivity, which is the live frontier.  The inhabitation
question --- does a concrete $\sigma$-orthocomplete non-Boolean OML
genuinely exercise descent? --- was sharpened by rerunning Navara's
product/constancy construction over a concrete stateful
$\mathrm{MO}_2$ block in place of his stateless one.
$\sigma$-orthocompleteness \emph{survives} the substitution (his
closure step is state-free).
%
\textbf{UPDATE 2026-06-10 ($(\star)$ resolved YES; concreteness open).}
Reading Navara p.~428 in full: $\mathcal{L}_2$ is a \emph{sublogic of}
the product $W$ (order = restriction), closed under orthocomplements
\emph{in} $W$, with countable orthogonal joins computed \emph{in} $W$.
Hence $a\,|\,C \not\perp b\,|\,C$ transfers from $\mathrm{MO}_2$, the
witness $p=\bigvee_n b\,|\,C_n$ satisfies the interleaving condition
$(\star)$ in $\mathcal{L}_2$, and \textbf{segregation is refuted} ---
$\mathcal{L}_2$ carries genuine non-distributive descent \emph{structure}.
\textbf{But this does not yet falsify the conjecture and is not yet a
contribution:} the conjecture is about \emph{concrete} OMLs, and $(\star)$
does not give concreteness.  The crux is the \textbf{horizontal-sum step}
($V$ = pasting of $T\times\mathrm{MO}_2$ copies; pasting destroys states,
the Greechie mechanism), which ``sub-OMP of concrete is concrete'' does
not cover.  \textbf{The open question is now: is $\mathcal{L}_2$ concrete?}
--- the \texttt{/audit full} target (concreteness primary, novelty-vs-localic
secondary).  Concrete $\Rightarrow$ conjecture false + candidate contribution
(first standalone lead since 2026-05-18); not concrete $\Rightarrow$ another
$L(H)$-analogue, conjecture stands.  Earlier ``inhabited as stated / false as
literally worded / HINGE the only open step'' is \emph{superseded}: the HINGE
resolved YES, but concreteness, not transfer, decides.  Canonical statement:
\texttt{open\_questions/descent\_axis\_residue\_post\_kill.md}; derivation
history: \texttt{open\_questions/verification/inhabitation\_check.md}.

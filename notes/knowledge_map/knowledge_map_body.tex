...
Pt\'{a}k--Pulmannov\'{a}~(1994) prove that if an orthomodular
lattice possesses a unital set of subadditive probability measures,
it must be Boolean.  The gap between ``too weak'' (allows
non-$\sigma$-additive states) and ``too strong'' (forces Booleanity)
appears structurally robust.  Bunce--Wright~(1992) solve the
Mackey--Gleason problem for von Neumann algebras (a different
question); Chetcuti--Dvure\v{c}en\-skij~(2003/2005) use FA states
to characterize completeness of inner product spaces (also
different).  Vor\'{a}\v{c}ek--Pt\'{a}k~(2023) confirms no
breakthrough on the extension question proper.  All
exotic OML state spaces use Greechie pasting (Wilce 2009, Handbook
of Quantum Logic Ch.~24).  Needs a new idea, not more reading.

...
| the proved product-Ulam witness carrier is NOT a lattice (fails `MeetsExist`) | **Lean-certified** (`UlamWitnessLatticeGap` `witness_carrier_not_lattice`, axioms `[propext, Classical.choice, Quot.sound, slab0_not_mem]`, no `sorryAx`) modulo one cited fact | proved: the meet of `coreA,coreB` would be forced to `M×{0}` (choice-free reduction `greatest_lowerBound_eq_slab`), which is not in the carrier (Cor 4.1, cited `axiom slab0_not_mem`). Pins the OMP/OML boundary: the ZFC witness is an OMP; the OML form is the open `PsiOML` (paper §9 conjecture = grand Φ). §3 Normal-Form formalization would discharge the cited axiom (flagged next unit) |
...
The first exact bounded relation-section quotient was only retained-side
...
vector, but the audited variant still has zero bad retained-side kernels (payload
`39bd3fe0...`). Coarse-shadow grammar is refuted; exact section-congruence
minimization is closed negatively. A certified actual generated-event escape
now exists, so conditional re-basing is active.

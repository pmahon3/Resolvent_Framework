...
### Iteration 291 — restricted actual mixed-join control
- The exact quotient generated by the cylinders of maximal old lowers
  `15250`, `16786`, their complements, and actual `Neg-023` has 14 truth
  atoms. No `g` event is seeded.
- Target-side closure stabilizes at 12 events; both-copy closure stabilizes at
  16. Both are lattices and orthomodular; the 16-event family has centre
  `{0,1}`. **Executable verified — sampled finite scope.**
- In both closures the least upper bound of the maximal lower cylinders is
  exactly `Neg-023`. It is noncylindrical in both orientations. Their literal
  union is exactly the recorded `g` cylinder and is not generated.
- Producer `dec02775...`, payload `f1df1685...`; emit/verify runtimes about
  219 seconds each. Verification recomputes the producer, not an independent
  implementation.
- **Refuted:** the two maximal lowers, both orientations, OML laws, and
  trivial centre suffice to force cylindrical re-basing.
- **Nonclaim:** the control omits the full old 18676-event copy, state audits,
  and every sigma/global gate; it does not refute `ARR-CYL`.
- **Pivot:** classify one-old-event extensions of the 16-event control and

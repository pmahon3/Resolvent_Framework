...
6. **Unresolved theorem:** the 256 first-round events generated by
   `AR-REB-001` admit a finite provenance-sensitive normal form whose
   transitions cover complement/disjoint union and whose critical-pair
   diamonds are order-independent.
7. **First action:** classify the 256 capped `AR-REB-001` events by exact
   generation provenance, target/cut incidence, section roots and endpoint
   behaviour. Test the smallest diamond through `Neg-023`/`And-010`. Do not
   raise the closure cap until the existing events distinguish a live finite-
   grammar hypothesis.

...
6. **Unresolved theorem:** `ARR-CYL`. For the actual `Neg-023` relation in
...
7. **First action:** construct or characterize the actual two-copy sublattice
   generated by the old cylinders and `Neg-023` far enough to determine `h`;
   test whether all its opposite fibres are empty or full. If not, preserve
   its exact partial-fibre profile as the first genuine mixed repair. Do not
   begin third-copy complementary-cover enumeration before this lifting gate.

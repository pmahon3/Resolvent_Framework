...
Construct or characterize the actual adjacent-pair sublattice generated by
the old cylinders and `R` far enough to determine the cut join `h`. Test
whether every opposite fibre of `h` is empty or full. Do not launch a
third-copy complementary-cover census before this gate is settled.

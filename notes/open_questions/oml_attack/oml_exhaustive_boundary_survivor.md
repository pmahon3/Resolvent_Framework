...
extrema and the orthomodular equation reduce to those controls.  It also
gives the maximal-block classification.  If an event is compatible with
every event of one displayed inflated block, any failure to lie in that
block is witnessed against one outside atom or one fibre coefficient; that
pair again lies in a Boolean algebra with at most four atoms.  The checked
compatibility envelopes then force the event into the displayed block.
The same argument applies to the two finite blocks.  Hence every maximal
compatible set is contained in, and therefore equals, one of the five named
blocks.  This proves exactly five maximal blocks for every nondegenerate
concrete Boolean sigma-algebra `A`.

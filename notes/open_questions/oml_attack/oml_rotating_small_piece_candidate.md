...
proved conditional on the later compatibility/state gates. Gate N passed in
§8 of the closure-calculus note; Gate L failed in §9. The candidate is not a
lattice.⟧
...
Gate N **passes** by Theorem 8 of the closure-calculus note. Universal
transversal membership supplies a type-uniform anchor, a disjoint cylinder
adds all binary supports, and certified local events patch the countable
exceptional columns. The exact parameterized normal form is therefore
available. Gate L **fails**: `cyl(T₁)` and `G₁₂` have no join by Theorem 10
of that note. Gates M, C, Z, F, and Φ are not entered.

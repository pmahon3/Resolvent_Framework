...
**Two-fibre selection update.**
[`oml_two_fibre_monodromy.md`](oml_two_fibre_monodromy.md) shows that the
smallest jointly coherent atom pair has disjoint propagation supports.
Finite completion remains centre-free, but the compatibility relation is
rectangular; independent finite-trace point replacement is now
Lean-certified.

**2026-07-12 centre-removal audit.**
[`oml_irreducible_boundary_test.md`](oml_irreducible_boundary_test.md)
shows that a two-block interface is central in the OML generated by the two
blocks. The raw three-block crossed-interface model fails, but its 56-event
completion is a centre-free concrete σ-complete OML with seven maximal
blocks and every overlap-generated boundary equal to its full block. It
also supplies the finite skeleton later used to sharpen compact globalization and separates it from

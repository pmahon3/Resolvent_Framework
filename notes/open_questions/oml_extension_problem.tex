...
$\sigma$-additive measure on the dual space (PR)?  This dual-space
reading of PR is \emph{stricter} than the lattice-level realisation
Gleason supplies (which is the sense in which Paper~II's PR is met for
$L(H)$, with the MB dual never entering): \S\ref{sec:lh} shows the
dual-space passage \emph{fails} for every normal state, so even $L(H)$
does not clear this stronger, dual-space refinement of PR on the
extension axis.  Paper~II's L(H) realisation claim is untouched --- the
failure is of the strictly stronger dual-space notion, not of the
lattice realisation.  Under what conditions, then --- and is descent a
free choice (as in the Boolean case) or a structural consequence of the
algebra and state together?

...
For \emph{concrete} (set-representable) OMLs the lattice meet is
set intersection, so meet-zero coincides with orthogonality, and
the extension axis reduces to the classical marginal /
Pitowsky non-contextuality problem.  For \emph{non-concrete} OMLs
($\mathrm{MO}_3$, $L(H)$) it is strictly stronger and can fail
universally.  In the \emph{finite} case it is a decidable linear
program, not a structurally resistant frontier.  What remains open
is the \emph{infinite} case --- $L(H)$ and the actual
$\sigma$-additive measure on $S_0(A)$ --- to which the finite
counterexample does not speak.
...
in a non-distributive OML is non-empty.  In the finite case it is a
decidable linear program; it can fail for \emph{every} state
($\mathrm{MO}_3$).  This is not a structurally resistant frontier
and does not require a new idea.  The earlier framing of this axis
as ``the Pt\'{a}k--Pulmannov\'{a} frontier needing a new idea'' was
mistaken --- Pt\'{a}k--Pulmannov\'{a}~\cite{PtakPulmannova1994}
...
\medskip\noindent
\textbf{Descent axis: genuinely open.}  Whether a measure on
$S_0(A)$ concentrates on the physical points $P(A)$, and the
analogue of Paper~I's ``$\sigma$-additivity $\Leftrightarrow$
concentration,'' cannot even be stated rigorously without
$\sigma$-completeness and continuity hypotheses, because the
McDonald--Bimb\'{o} duality is finitary (the countable-join
identity fails). This is the live residue.

\medskip\noindent
\textbf{The infinite extension case.}  The finite counterexample
does not speak to $L(H)$ or to the actual $\sigma$-additive measure
on $S_0(A)$ for an infinite OML.  Whether the meet-zero/orthogonal
gap produces the same universal failure infinitely, or whether
$\sigma$-completeness changes the picture, is open.
...
Pitowsky feasibility de-mystifies it.  For $L(H)$, Gleason resolves
both axes at once.

\medskip\noindent
\textbf{Status.}\enspace Extension axis: settled (finite),
open (infinite).  Descent axis: open, contingent on a rigorous
finitary-to-$\sigma$ bridge.  Not a current active lead.

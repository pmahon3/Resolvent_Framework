...
\begin{description}[style=nextline, font=\normalfont\bfseries,
  ...
  \item[$\text{PR}_{\text{dual}}$]
    ...
    followed by descent.
\end{description}
The containment $\text{PR}_{\text{dual}} \subsetneq
\text{PR}_{\text{lat}}$ is a \emph{real, strict, witnessed}
stratification: $L(H)$ normal states clear $\text{PR}_{\text{lat}}$
(Gleason) and fail $\text{PR}_{\text{dual}}$ (\S\ref{sec:lh}: no charge
on $S_0(L(H))$ exists, a fortiori none concentrating).  This is the
cleanest statement of the infinite-extension result in the realism
vocabulary, and it lets one assert \emph{both} grades at once --- Gleason
clears $\text{PR}_{\text{lat}}$ for $L(H)$ while $\text{PR}_{\text{dual}}$
fails --- without suppressing either.  In the Boolean case the two grades
coincide (Stone gives $\hat\mu$ unconditionally; descent $\Leftrightarrow$
$\sigma$-additivity), which is why Paper~II could write a single PR.  The
distinction is philosophically neutral: a Born-rule realist reads off
...
strict stratification with the descent half not yet shown independent ---
a vocabulary refinement, not a new theorem.

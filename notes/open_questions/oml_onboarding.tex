...
representation. We close with the first concrete move.
\end{abstract}
...
singular state fails to extend, then \emph{no} state on $L(H)$ extends; the
singular sliver is the only candidate witness for descent doing work
independent of extension, so its failure would settle that question
negatively. If a singular state \emph{does} extend, it is the first
concrete object that extends but may fail to concentrate, handing
branch~(a) its motivating example.
...
is vacuous ($s(e)=0$ on all finite-dimensional $e$). But ``the line route is
...
finitary-to-$\sigma$ bridge as (a). The concrete object to examine is a
finite family of infinite-dimensional, pairwise-trivially-intersecting
subspaces of $H$, and whether orthoadditivity of a singular state forces
$\sum_i s(a_i) > 1$ on it.

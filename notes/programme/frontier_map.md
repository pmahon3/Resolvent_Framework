...
cosmetic; receipt `notes/open_questions/oml_attack/PROOF_READ_2026-07-10_attack_s9.md`).
...
`notes/open_questions/oml_attack/SKELETON_A_READ_2026-07-10_receipt.md`. **s14:** §10
...
oracle 10/10; proof-read CLEARED s16 — SOUND, no wrong steps,
424,509/424,509 from-scratch checks, receipt
`notes/open_questions/oml_attack/PROOF_READ_2026-07-11_attack_s11.md`⟧ — 2BR two-block rescue (open locus = ≥3-block
...
slice **finite overlap-generated boundary at every block $\Rightarrow\Phi$**.
...
**s52 independent arbitrary-base review and smallest twist:**
The five-maximal-block and sigma-completeness claims survive independent
hand review.  A normal-form/finite-reduction proof reduces every binary
relation to the Boolean algebra generated by two fibre coefficients, hence
to at most four atoms; countable disjoint unions reduce to a finite outside
head plus a sigma-union in the common fibre.  This is a hand proof, with the
existing 2--4 atom controls as bounded executable support.  The smallest
`P(2)` triangle twist has eight edge labellings: four odd-parity labellings
have nonidentity holonomy and collapse the shared interval, while the four
faithful even-parity labellings are gauge-trivial.  This bounded test does
not classify larger or proper-interface twists.  Next gate: a proper shared
...
The canonical maximal envelope is `T=R∩cyl_right(18322)`. It is noncylindrical on both
sides, with a principal left old kernel and the original incomparable
right-kernel pair. Fixed-cut forced-top descent is well-founded; global
termination now reduces to controlling new source cuts generated by mixed
bridges. The first one-step executable attempt was excluded for artifact
hash mismatch and a wrong nonemptiness predicate.
The complete immediate literal-bridge census instead yields 498 occurrences
and 488 exact roots (486 new). It refutes a frozen-five parent grammar: 474
roots require an outside old parent. The 27 coarse buckets do not compress

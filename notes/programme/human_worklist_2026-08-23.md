...
## H1 — Confirm the Andersen–Jessen hypothesis match  *(RESOLVED 2026-08-24 — CONFIRMED)*

> **RESOLVED 2026-08-24 (Claude). `EvalSurjective` CONFIRMED — and no longer a
> hand check: it is machine-checked.** `formalization/staging/H1_EvalSurjective.lean`
> proves `AJ_evalSurjective` against the real `QuerySystem.Omega` / `eval`
> definitions, receipt `[propext, Classical.choice, Quot.sound]`.
>
> The reasoning below is right, for the reason given. One correction to the
> framing: `Omega` is NOT the bare product `∏ₖ Xₖ` — it is the subtype of
> *coherent* families (`x i = π hij (x j)`). That is exactly where such an
> argument can fail, since surjectivity onto a projective limit can fail even
> with every level nonempty. It does not fail here: with
> `Outcomeₙ = X₀ × ⋯ × Xₙ` and bonding maps = initial-segment restriction,
> coherence is automatically satisfiable (the family is generated by a single
> `g : ∀ k, Xₖ`, and `π_refl`/`π_trans` discharge by `rfl`). Nonemptiness of
> each `Xₖ` genuinely suffices, supplied by `X_thick`.
>
> Per the Consequences branch below: **H2/H3 proceed as written.**
>
> Scope: this closes the *hypothesis match* only. It does NOT close the
> trace-measure / projective-system layer — see
> `notes/open_questions/aj_tower/SCOPE_trace_projective_layer.md`. The
> refutation as a whole is still not kernel-checked.

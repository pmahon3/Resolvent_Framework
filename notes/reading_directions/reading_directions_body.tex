...
\subsubsection*{Ellerman, Partition Logic and Information}

\noindent\textbf{Primary:} D.~Ellerman, \emph{New Foundations for
Information Theory: Logical Entropy and Shannon Entropy}, Springer
(2021).  See also Ellerman, ``The Logic of Partitions,'' \emph{Review
of Symbolic Logic} 3 (2010), 287--350.

\medskip\noindent
Ellerman develops a ``logic of partitions'' dual to the Boolean logic
of subsets.  Where subsets model definiteness (an element is in or
out), partitions model indefiniteness (elements are distinguished or
not).  The basic objects are ``dits'' (distinctions) rather than
``bits.''  Shannon entropy is derived from a more primitive ``logical
entropy'' defined on partitions.  Partition refinement is the natural
order.

\medskip\noindent\textbf{Discriminating question:} Does partition
refinement give a non-trivial functor to charge refinement --- i.e.,
does the passage from partitions to the Boolean algebra they generate,
equipped with a finitely additive charge, preserve or reveal structure
not visible in the algebra alone?  Or is the dit/bit translation just
relabeling entropy without producing new content about the
FA/$\sigma$A boundary?  The test: find a theorem in partition logic
whose translation into Boolean-algebra language says something about
extension that is not already in Halmos/Yosida-Hewitt.

...
\emph{Status}: working note.  Definitions and the first bridge proposition are in place;
...
\begin{remark}[Upper-directedness in general]\label{rem:upper-directed-general}
Full upper-directedness across all lags requires a common refinement for
queries at incommensurable lags $(d,\tau)$ and $(d',\tau')$ with
$\tau \neq \tau'$.  This holds if the index set is enriched to include
queries at lag $\gcd(\tau, \tau')$ or finer.  For the purposes of this
note we work within a fixed lag $\tau$ or note explicitly when the full
index set is needed.
\end{remark}
...
Theorem (Paper~1, Theorem~6.1), any such compatible family --- given
sequential upper-directedness of the delay query system --- determines a
unique probability measure $P$ on $(\Omega, \Fcal)$ whose delay marginals
are $\{\nu_{d,\tau}\}$.  The content of Paper~1 therefore applies directly
once Propositions~\ref{prop:is-qs}--\ref{prop:realizability} and the
upper-directedness condition (Remark~\ref{rem:upper-directed-general})
are verified.
\end{remark}
...
\begin{enumerate}
  \item \textbf{Upper-directedness at incommensurable lags.}  Does the
    full delay query system (all $d, \tau$) satisfy sequential
    upper-directedness?  The lag-$\gcd$ construction suggests yes, but
    this requires a careful argument about the measurability of the
    resulting refinement maps.
  ...
  \item \textbf{Compatible marginals from stationarity.}  If $P$ is a
    ...
    for the refinement maps at incommensurable lags.

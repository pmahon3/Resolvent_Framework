...
Dynamic Mode Decomposition (DMD)~\cite{Schmid2010} and EDMD~\cite{Williams2015}
approximate the Koopman operator from time-series data.  The present framework
...
probabilistic extension of EDMD; the convergence result
(Proposition~\ref{prop:local-linear}) is the query-theoretic analogue of the
EDMD consistency theorems.
...
\begin{enumerate}
  ...
  \item \textbf{Compact outcome spaces and tightness.}
    ...
    embeddings map into $X^d \cong \mathbb{R}^d$, which is not compact.
    Understanding whether locally compact or cylindrically tight conditions
    on $P$ suffice to recover the Paper~0 corollary in the delay setting
    is the concrete form of the open SP2 question for this paper.

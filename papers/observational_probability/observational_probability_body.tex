...
$(d',\tau') \preceq (d'',\tau'')$.  More generally, every \emph{finite}
family of delay queries has a common refinement.
\end{proposition}
...
\textbf{Finite case.}
Induction on the family size, applying the pairwise case at each step.
\end{proof}

\begin{remark}[The full system is not sequentially upper-directed]\label{rem:not-seq-udir}
The abstract framework of Paper~1 requires \emph{sequential} upper-directedness:
every countable sequence of queries must have a single finite common refinement.
The full delay query system does \emph{not} satisfy this condition.

\textbf{Counterexample.}  Consider the sequence $Q_{n,1}$ for $n = 1, 2, 3, \ldots$\
Any common refinement $Q_{d'', \tau''}$ would require $\tau'' \mid 1$,
forcing $\tau'' = 1$, and then $(n-1) \cdot (1/1) = n-1 < d''$ for all $n$,
which is impossible for any finite $d''$.

The correct scope of the extension theorem is therefore the
\emph{bounded fixed-lag subsystem}: for fixed lag $\tau_0 \in \mathbb{N}_{>0}$
and fixed maximum dimension $N \in \mathbb{N}_{>0}$, the subfamily
\[
  \mathcal{Q}_{N,\tau_0} = \{Q_{d,\tau_0} : 1 \leq d \leq N\}
\]
is a finite chain (totally ordered by dimension) and is therefore
sequentially upper-directed with $Q_{N,\tau_0}$ as a universal upper bound.
The Observational Extension Theorem (Paper~1, Theorem~6.1) applies to each
$\mathcal{Q}_{N,\tau_0}$.  The full delay system is the inductive limit
over $N$ and $\tau_0$ of these bounded subsystems.

This distinction is analogous to the classical Kolmogorov extension theorem,
which applies to consistent families of finite-dimensional distributions but
does not directly extend to uncountably many projections simultaneously.
\end{remark}
...
\begin{remark}[Scope of the extension theorem]\label{rem:ext-scope}
Propositions~\ref{prop:is-qs}, \ref{prop:upper-directed-general},
\ref{prop:realizability}, and \ref{prop:compat} verify the axioms of a
query system and realizability for the delay query system.  In view of
Remark~\ref{rem:not-seq-udir}, the Observational Extension Theorem
(Paper~1, Theorem~6.1) applies to each bounded fixed-lag subsystem
$\mathcal{Q}_{N,\tau_0}$: any compatible family $\{\nu_{d,\tau_0}\}_{d \leq N}$
determines a unique probability measure on the projective limit
$\varprojlim_{d \leq N} X^d$ recovering the prescribed marginals.

In particular, starting from any $P$ on $\Omega = X^{\mathbb{Z}}$, the
induced marginals $\nu_{d,\tau} = (\mathrm{eval}_{d,\tau})_\sharp P$ are
compatible (Proposition~\ref{prop:compat}), and the extension theorem
recovers $P$ on each $\mathcal{Q}_{N,\tau_0}$.  Taking the inductive limit
over $N$ and $\tau_0$ recovers $P$ on the full system.
\end{remark}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Operator theory specialization}\label{sec:delay-operators}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
local linear approximation scheme (Section~\ref{sec:delay-operators}) is the

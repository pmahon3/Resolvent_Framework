...
\begin{abstract}
Paper~III introduces two witnesses for finite-sample reconstruction: the
...
Under Paper~III's standing hypotheses (compact smooth $X$, smooth positive $\mu$,
...
Paper~III (\S4--5) constructs an empirical witness $\hat\delta(L,n)$ and proves
...
defined in Paper~III~(Thm~5.4) could equivalently be stated as the first $L$
at which the empirical collision entropy $\hat H_2(\nu_L^{(n)})$ exceeds a
threshold calibrated to $n$: both witnesses detect the same transition.
\end{remark}

\begin{remark}[Relation to Paper~III witnesses]
The three witnesses now in scope are:
\begin{enumerate}[label=(\roman*)]
  \item $\hat\delta(L,n)$ --- the algebraic witness (Paper~III, \S5);
  \item $\hat d_L(x,x')$ --- the delay-map separation (Paper~III, \S6);

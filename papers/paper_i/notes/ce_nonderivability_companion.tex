...
same obstruction shows that collective exhaustion --- the necessary and sufficient
condition for $\sigma$-additive extension in a directed system of Boolean algebras
\cite{mahon2025} --- cannot be derived from any first-order structural condition.
\end{abstract}
...
admissibility condition for $\sigma$-additive extension in directed systems of Boolean
algebras studied in \cite{mahon2025}.  Section~\ref{sec:rep} raises the representation
...
In \cite{mahon2025}, collective exhaustion (CE) is introduced as the necessary and
sufficient condition under which a compatible finitely additive family on a directed
system of Boolean algebras extends to a $\sigma$-additive measure on the observable
$\sigma$-algebra.  CE is a valuation-layer condition: it requires that whenever a
decreasing sequence of events has empty intersection, the charges assigned to those
events converge to zero.

Proposition~\ref{prop:not-fo} implies:
...
forces, the extension property characterized in \cite{mahon2025} by collective
exhaustion.
\end{corollary}

\noindent
The paper \cite{mahon2025} establishes this at the level of query systems: no
first-order structural condition in the query-system language implies CE.
Corollary~\ref{cor:ce-not-fo} shows the obstruction is not an artifact of that
formalism.  It already appears in the more primitive setting of Boolean algebras
with finitely additive charges, where countable additivity is not first-order
axiomatizable.

Foundational disputes about probability are not terminological.  They concern
which extra structure licenses the passage from finite coherence to probability,
and Proposition~\ref{prop:not-fo} shows that some such extra structure is
logically unavoidable: no accumulation of first-order conditions can close the
gap between finite additivity and $\sigma$-additivity.

...
\begin{proposition}[Countable additivity is not first-order]
\label{prop:not-fo}
...
enumeration $q : \N \to \Q$, and let $\delta_n$ be the Dirac measure at
...
filter.  By \L{}o\'{s}'s theorem, the ultraproduct $\prod_\mathcal{U}(E,\delta_n)$
...
$\ell(A_k) = 1$ for all $k$.  So $\ell$ is not $\sigma$-additive and does not
extend to a $\sigma$-additive measure on $\sigma(E)$.  Therefore the
ultraproduct does not satisfy $T$.  This contradicts \L{}o\'{s}'s theorem
applied to the assumption that each $(E, \delta_n) \models T$.
\end{proof}

\begin{remark}
For complete model-theoretic precision: the ultraproduct $\prod_\mathcal{U}(E,\delta_n)$
...
In the companion paper, collective exhaustion (CE) is the exact condition under
which a compatible finitely additive family extends to a $\sigma$-additive measure
on the observable $\sigma$-algebra.  Proposition~\ref{prop:not-fo} therefore
implies:
...
additive charge, no first-order condition implies the extension property
characterized in the companion paper by collective exhaustion.
\end{corollary}
...
encode coherence, but not the countable-exhaustion behavior required for probability.
...
It may be true in some form, or may require allowing approximating nets rather than
literal ultraproducts.  The Yosida--Hewitt decomposition identifies the purely

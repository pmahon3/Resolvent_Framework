...
condition quantifies over countable sequences, unavailable in first-order logic.  The
probability logic community has treated the non-first-order character of $\sigma$-additivity
as background knowledge: Hoover~\cite{Hoover1978} requires an infinitary continuity rule
to axiomatize $\sigma$-additive probability logic, Keisler~\cite{Keisler1985} notes that
$\sigma$-additivity does not fit finitary Henkin constructions, and Fagin, Halpern, and
Megiddo~\cite{FaginHalpernMegiddo1990} remark without proof that countable additivity
``cannot be expressed by a formula'' in the natural language of Boolean algebras with
rational charge comparisons.  The sharper question --- whether countable additivity
admits a first-order \emph{axiomatization} by any set of sentences in that language,
not merely a single formula --- does not appear to have been settled by an explicit
...
we have $\ell(E_n) \to 0$.  CE is a valuation-layer condition: it constrains how
charges behave across the directed system, not the Boolean-algebraic index structure.
It is the necessary and sufficient condition under which a compatible finitely additive
family extends to a $\sigma$-additive measure on the observable $\sigma$-algebra
...
for the non-axiomatizability conclusion.  A stronger question concerns the scope of
this phenomenon:

\begin{quote}
\emph{Which purely finitely additive charges on a Boolean algebra arise as pointwise
ultralimits of $\sigma$-additive probabilities on the same algebra?}
\end{quote}

On a $\sigma$-algebra the answer is: none.  If $(\mu_n)$ is a sequence of
$\sigma$-additive probabilities on a $\sigma$-algebra $\Sigma$ and $\mathcal{U}$
is any ultrafilter on $\mathbb{N}$, then $\ell(A) = \lim_\mathcal{U}\mu_n(A)$
is itself $\sigma$-additive.  The reason is that for disjoint $(A_k)$ with
union $A$, the identity $\mu_n(A) = \sum_k \mu_n(A_k)$ holds exactly for each $n$,
so the limit and sum interchange without difficulty; see Dunford--Schwartz
\cite{DunfordSchwartz1958}, IV.9.8.  Consequently the question is entirely
about Boolean algebras that are not $\sigma$-complete.

On a general Boolean algebra the situation is open.  The Yosida--Hewitt
decomposition \cite{YosidaHewitt1952} decomposes any finitely additive charge
into a $\sigma$-additive and a purely finitely additive part (orthogonally) but
says nothing about approximation.  For ultrafilter-generated $\{0,1\}$-valued charges the present
proof already gives a positive answer: each such charge is the ultralimit of the
Dirac masses on the finite-cofinite algebra.  Whether every purely finitely additive
charge on a Boolean algebra admits such a representation --- and in particular whether
the answer depends on set-theoretic hypotheses about ultrafilters --- is left open.
Duanmu and Weiss \cite{DuanmuWeiss2018} show that on totally bounded separable
metric spaces every finitely additive Borel probability is a weak limit (against
bounded uniformly continuous test functions) of $\sigma$-additive probabilities,
but remark that setwise ultralimit approximation fails for ultrafilter charges on
discrete sets (Example~5.14), confirming that the pointwise ultralimit question
is genuinely delicate.

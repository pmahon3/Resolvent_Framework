...
on the same algebra.  On a $\sigma$-algebra the answer is none: the identity
$\mu_n(A) = \sum_k \mu_n(A_k)$ holds exactly for each $n$, so any pointwise
ultralimit of $\sigma$-additive probabilities is itself $\sigma$-additive
\cite{DunfordSchwartz1958}, IV.9.8.  On a general Boolean algebra the question
is open: ultrafilter-generated $\{0,1\}$-valued charges are representable (the
present Dirac construction gives this), but whether the same holds for all purely
finitely additive charges --- and whether the answer is set-theoretically sensitive
--- is not known~\cite{YosidaHewitt1952,DuanmuWeiss2018}.

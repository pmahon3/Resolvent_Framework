...
topological assumptions.  The \emph{Stone route} assumes only finite additivity:
compactness of the Stone space manufactures $\sigma$-additivity on the compact
completion, and CE is the support condition that descends the measure back to the
...
The Carathéodory route begins from $\sigma$-additive marginals: CE is the
condition whose satisfaction licenses that starting point.  A finitely additive
family deserves to be treated as a probability system precisely when it does not
assign persistent mass to events the refinement hierarchy sees as empty ---
and Theorem~\ref{thm:sp1} shows that is exactly CE.
...
Stone route derives that status from compactness and the support condition.

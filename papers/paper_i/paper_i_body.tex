...
Section~\ref{sec:bridge} draws out CE's unified meaning across both constructions and characterises the Stone space.  The main results
...
\subsection{CE has two faces, one meaning}

Collective exhaustion is one condition with two descriptions.  Algebraically,
it says that a compatible family does not assign persistent mass to events the
refinement hierarchy sees as empty: whenever $E_n \searrow \emptyset$ in
$\mathcal{C}$, the mass $\mu(E_n) \to 0$.  Geometrically, it says that the
Baire measure $\hat{\mu}$ on the Stone space $\mathrm{St}(\mathcal{C})$ is
supported on $\mathrm{pure}(\Omega)$, the principal ultrafilters --- the
observer's actual world, not its ideal completion.

The Yosida--Hewitt decomposition makes the identity precise.  Every bounded
charge decomposes uniquely as $\mu = \mu_c + \mu_p$, where $\mu_c$ is
$\sigma$-additive and $\mu_p$ is purely finitely additive.  Under Stone
duality, $\mu_p$ is exactly the mass that has escaped to non-principal
ultrafilters (Proposition~\ref{prop:B2}).  CE is the condition $\mu_p = 0$:
algebraic convergence is honest, and the geometric measure is confined to
$\Omega$.  The two descriptions are one.

The observable extension theorem is the consequence.  When CE holds, the
Carathéodory argument produces a $\sigma$-additive extension directly, because
CE --- via Theorem~\ref{thm:sp1} --- is precisely the condition under which
finitely additive level data warrants $\sigma$-additive status.  The Stone
argument reaches the same measure from the other direction: compactness of
$\mathrm{St}(\mathcal{C})$ manufactures $\sigma$-additivity on the compact
completion, and CE returns it to $\Omega$ by forcing the measure off the
phantom ultrafilters.  Under shared hypotheses, both constructions coincide.
...
\noindent That both constructions produce the same measure is a consequence of
uniqueness, not of the constructions themselves.  A coherent family of
observations satisfying CE determines a unique probability measure on the
observable $\sigma$-algebra.  CE is what makes that determination possible:
without it, finitely additive data may cohere locally without sustaining any
global probabilistic interpretation.  As \S\ref{sec:metatheorem} records, no
first-order structural condition can supply CE's role --- the gap it bridges is

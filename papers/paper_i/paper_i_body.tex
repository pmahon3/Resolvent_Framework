...
A \emph{direct Carathéodory construction} assumes compatible $\sigma$-additive marginals and builds the global measure directly without topological assumptions. Assumed $\sigma$-additivity may be relaxed to finite additivity, regained later via extension under a support condition.
...
A \emph{Stone construction} assumes only finite additivity leveraging exhaustion to extend: compactness of the Stone space manufactures $\sigma$-additivity on the compact completion where CE is the support condition that descends the measure back to the instantiation space.

The Stone space $\mathrm{St}(\mathcal{C})$ is the canonical compact Hausdorff completion of the observable structure: the space of all consistent distinction patterns extending the cylinder algebra.
\end{abstract}
...
Section~\ref{sec:setup} introduces query systems, cylinder sets, the observable $\sigma$-algebra, compatible charges, and the structural hypotheses (Surjective Evaluation, Discriminability).  Section~\ref{sec:cara} gives the direct extension theorem: given compatible $\sigma$-additive marginals, the global observable law follows by Carathéodory.  Section~\ref{sec:ce} establishes CE as the admissibility criterion that determines when finitely additive level data warrants that probabilistic status, proves its non-derivability from structural conditions, and shows that the underlying logical obstruction is not specific to the query-system language (\S\ref{sec:metatheorem}). Section~\ref{sec:stone} proves the Stone realization from finitely additive data. Section~\ref{sec:bridge} draws out CE's unified meaning across both constructions and characterises the Stone space.  The main results have been formalized in Lean~4 using Mathlib; source files are available in the accompanying repository.

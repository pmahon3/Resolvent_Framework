...
extension theorem: given compatible $\sigma$-additive probability marginals, it
constructs the global observable law directly.  Theorem~\ref{thm:sp1} identifies
CE as the condition under which finitely additive level data can be promoted to
the $\sigma$-additive marginals this route assumes.
...
from compactness.  Descending from the Stone space to $\Omega$ requires two
...
The Stone space $\mathrm{St}(\mathcal{C})$ is the canonical compact Hausdorff
completion of $\Omega$; CE's two faces --- algebraic convergence and geometric
support --- are the same condition seen from inside and outside the algebra
(\S\ref{sec:bridge}).

\paragraph{Organisation.}
Section~\ref{sec:setup} introduces query systems, cylinder sets, the observable
$\sigma$-algebra, compatible charges, and the structural hypotheses (Surjective
Evaluation, Discriminability).  Section~\ref{sec:cara} gives the direct extension theorem:
...
The two routes impose different structural conditions on the query system.
...
\noindent Discriminability ensures the Stone embedding is injective; it is
required by the Stone route (\S\ref{sec:stone}) but not by the Carathéodory
route.
...
yields a global measure directly on the instantiation space.  The input is a
query system satisfying Surjective Evaluation, sequential common refinements,
and common coarsenings, together with a compatible family of $\sigma$-additive
probability marginals.  Section~\ref{sec:ce} identifies the admissibility
condition under which finitely additive level data can be promoted to this
input.
...
\noindent CE is a valuation-layer condition: it constrains how charges behave,
not the Boolean-algebraic index structure.  The following example shows it is
not automatic.
...
We work first at the level of an abstract directed system of Boolean algebras,
writing $\mathcal{E}_i$ for the algebra at level $i$ and $\ell_i$ for its
charge; the query-system interpretation returns in \S\ref{sec:stone}.
...
finer level.  No level may assign persistent mass to events the system as a
whole sees as empty.

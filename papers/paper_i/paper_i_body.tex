...
Two independent routes establish the result.  The \emph{Carathéodory route} (\S\ref{sec:cara}) assumes $\sigma$-additive
marginals and constructs the global measure directly on the instantiation space.
Sequential common refinements compress any countable cylinder family to a single
level, reducing the $\sigma$-subadditivity check to a single level.
...
$\sigma$-additivity is not presupposed but derived from compactness.  Stone duality converts the
...
Discriminability, CE).  Section~\ref{sec:ce} establishes the CE theorem and
the non-derivability of CE.  Section~\ref{sec:cara} proves the Carathéodory route.
Section~\ref{sec:stone} proves the Stone route.  Section~\ref{sec:bridge}
draws the two routes together and discusses the Stone space.  The main results
...
condition for $\sigma$-additive extensibility.  We work first at the level of
an abstract directed system of Boolean algebras, writing $\mathcal{E}_i$ for
the algebra at level $i$ and $\ell_i$ for its charge; the query-system
interpretation returns in \S\S\ref{sec:cara}--\ref{sec:stone}.
...
Section~\ref{sec:ce} isolated the exact obstruction to $\sigma$-additive
extension and showed it is not removed by the structural hypotheses on the query
system alone.  We now turn to the constructive side.  Assuming $\sigma$-additive
marginals --- equivalently, CE at each level by Theorem~\ref{thm:sp1} ---
Carathéodory's theorem yields a global measure directly on the instantiation
space.
...
A third, topological route begins from finitely additive marginals and derives
$\sigma$-additivity from compactness.  Under \emph{surjective projective
uniform tightness} --- for every $\varepsilon > 0$ there exist compact sets
$K_i \subset \Out{i}$ with $\nu_i(\Out{i} \setminus K_i) < \varepsilon$ and
$\pi_{ij}(K_j) = K_i$ for all $i \leq j$ --- the Prokhorov theorem extends
finitely additive marginals to a $\sigma$-additive global measure.  This route
assumes topological structure on the outcome spaces and is complementary to
the Carathéodory and Stone routes, which require none.
\end{remark}
...
route, $\sigma$-additivity is assumed from the outset and CE is the condition
it amounts to in the present framework.  In the Stone route, compactness
manufactures $\sigma$-additivity on the compact completion, and CE is then
the support condition that descends it back to $\Omega$.  In both cases CE
is doing the same work: ensuring that no mass escapes the realised observational
world.

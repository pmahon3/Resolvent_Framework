...
$\mathbb{N}$, all event algebras to be the finite-cofinite algebra
$\mathcal{E}$, all refinement maps to be the identity, and each $\ell_i$ to
be the normalized finite-cofinite charge $\ell$ of Example~\ref{ex:finite-cofinite}.
Compatibility holds trivially.  The system is sequentially upper-directed and
every structural condition is satisfied.  By Example~\ref{ex:finite-cofinite},
$\ell$ is not $\sigma$-additive and admits no $\sigma$-additive extension;
hence every level fails $\sigma$-additive extension.
\end{proof}
...
finite-cofinite content $\ell$ of Example~\ref{ex:finite-cofinite} on $\mathbb{Q}$.
...
The finite-cofinite content $\ell$ (Example~\ref{ex:finite-cofinite}) is purely finitely additive ($\ell_c = 0$).
CE is exactly the condition that rules out a nonzero purely finitely additive
part: $\ell_p = 0$.
\end{proof}

\noindent
CE is not a structural consequence of the query system.  It is the additional
admissibility condition under which observational coherence furnishes
probability --- and it cannot be certified from within the finitary structure
of the observational hierarchy itself.  The ultrafilter-based content is the
canonical witness: it passes every finite consistency test yet assigns unit
mass to events whose infinite intersection is empty.

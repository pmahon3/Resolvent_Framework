...
The Yosida--Hewitt decomposition reveals why CE does the same work in both
constructions.  Every bounded charge decomposes uniquely as $\mu = \mu_c +
\mu_p$, where $\mu_c$ is $\sigma$-additive and $\mu_p$ is purely finitely
additive; under Stone duality, $\mu_p$ is exactly the mass escaped to
non-principal ultrafilters.  CE is the condition $\mu_p = 0$: algebraic
convergence is honest, and the measure is confined to $\Omega$.  The algebraic
and geometric descriptions are one.
...
uniqueness, not of the constructions themselves.  CE is what makes that
determination possible: it is the condition under which finitely additive data
attains the $\sigma$-additive status needed for the observable law to be
uniquely determined.  Without it, finitely additive data may cohere locally
without sustaining any global probabilistic interpretation.  As
\S\ref{sec:metatheorem} records, no first-order structural condition can supply
CE's role --- the gap it bridges is logically unavoidable.

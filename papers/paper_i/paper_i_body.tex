...
measure on the observable $\sigma$-algebra.  Starting from a query system
$(\iota, \leq, \Omega, \{\mathrm{Out}(i)\}, \{\mathrm{eval}_i\}, \{\pi_{ij}\})$
and a compatible family of charges $\{\mu_i\}$, we identify the necessary and
sufficient condition for the family to extend to a $\sigma$-additive global
measure: \emph{collective exhaustion} (CE), the condition that no level can
...
Paper~\cite{mahon_paper0} uses $(\mathcal{Q}, \preceq)$ for the index set and
$O_Q$, $\pi^{Q_2}_{Q_1}$, $\nu_Q$ for the outcome spaces, refinement maps, and
charges.  The present notation ($\iota$, $\mathrm{Out}(i)$, $\pi_{ij}$, $\mu_i$)
is equivalent under the renaming $\iota = \mathcal{Q}$, $\leq\, =\, \preceq$;
all results translate directly.
\end{remark}
...
The results of this section admit a categorical articulation that further
clarifies why CE belongs irreducibly to the valuation layer.

The directed system $(\mathcal{I}, \{\mathcal{E}_i\}, \{\pi_{ij}\})$ is a
category $C(\mathcal{I})$ whose objects are the Boolean algebras and whose
morphisms are the refinement maps.  Equip $C(\mathcal{I})$ with the Grothendieck
topology $J$ in which a sieve $S$ on $\mathcal{E}_i$ covers $i$ if the levels
in $S$ collectively witness set-theoretic vanishing.  CE is the condition that
the charge presheaf $F_\ell : i \mapsto \ell_i$ is a sheaf for $J$ --- a
strictly stronger demand than the subcanonical condition.

The finite-cofinite content fails CE yet satisfies the sheaf condition for $J$.
The reason: for the sequences responsible for CE failure, no term is ever empty
as an element of $\mathcal{E}$, so no sieve can witness the vanishing
...
CE does become a genuine sheaf condition on the projective system of Stone
spaces $\{S(\mathcal{E}_i)\}$: the charge presheaf is a sheaf for the compact
Hausdorff topology if and only if the charges are tight, which by the extension
criterion is equivalent to CE.
\end{remark}
...
Kolmogorov's theorem constructs a probability measure on $S^T$ from consistent
...
See~\cite{mahon_paper0}, \S7, for the full development.
\end{remark}

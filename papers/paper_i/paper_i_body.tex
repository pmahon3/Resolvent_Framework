...
Section~\ref{sec:setup} introduces query systems, cylinder sets, the observable $\sigma$-algebra, compatible charges, and Surjective Evaluation.  Section~\ref{sec:cara} gives the direct extension theorem: given compatible $\sigma$-additive marginals, the global observable measure follows by Carathéodory.  Section~\ref{sec:ce} establishes CE as the admissibility criterion that determines when finitely additive level data warrants that probabilistic status, proves its non-derivability from structural conditions, and shows that the underlying logical obstruction is not specific to the query-system language (\S\ref{sec:metatheorem}). Section~\ref{sec:stone} proves the Stone realization from finitely additive data, introducing Discriminability as the condition that makes the Stone embedding injective. Section~\ref{sec:bridge} draws out CE's unified meaning across both constructions and characterises the Stone space.  The main results have been formalized in Lean~4 using Mathlib; source files are available in the accompanying repository.  This paper is the first step in a programme deriving canonical dynamics, state-space reconstruction, and certifiability from observational data; the further steps are carried out in the companion papers.
...
Section~\ref{sec:cara} showed that compatible $\sigma$-additive marginals extend directly to a global observable measure.  This section identifies the admissibility condition that determines when finitely additive level data deserves that probabilistic status.

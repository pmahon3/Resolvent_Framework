...
of finitely additive charges on the cylinder algebra. The result is illustrated by two constructions. 

A \emph{direct Carathéodory construction}  assumes compatible $\sigma$-additive marginals 
and builds the global measure directly  without topological assumptions. Assumed 
$\sigma$-additivity may be relaxed to finite  additivity, regained later via extension under a support condition.

The necessary and sufficient condition for $\sigma$-additive extension is \emph{collective
exhaustion} (CE): no level can assign persistent mass to events the system as a
whole sees as empty.  The fact that CE is not derivable from any structural condition on the
query system is precisely locate by the finite-cofinite counterexample and a model-theoretic 
argument via \L{}o\'{s}'s theorem.

A \emph{Stone construction} assumes only finite additivity leveraging exhaustion to extend: 
compactness of the Stone space manufactures $\sigma$-additivity on the compact completion 
where CE is the support condition that descends the measure
back to the instantiation space.  

Lastly we note the Stone space $\mathrm{St}(\mathcal{C})$ is the canonical compact Hausdorff
...
The result is established by two constructions with different starting
assumptions.

The \emph{Carathéodory construction} (\S\ref{sec:cara}) is the direct
local-to-global extension: given compatible $\sigma$-additive probability
marginals, it builds the global observable law directly.  Theorem~\ref{thm:sp1}
identifies CE as the condition under which finitely additive level data can be
promoted to that starting point.

The \emph{Stone construction} (\S\ref{sec:stone}) assumes only finitely additive
compatible charges and derives $\sigma$-additivity from compactness.  Descending
from the Stone space to $\Omega$ requires two conditions: discriminability ensures
the Stone embedding is injective, and CE appears as a support condition --- the
...
$\sigma$-subadditivity step in the Carathéodory construction.
...
required by the Stone construction (\S\ref{sec:stone}) but not by the
...
The Stone construction shows that $\sigma$-additivity can instead be derived.  Starting
...
The Yosida--Hewitt decomposition identifies the algebraic and geometric faces
of CE: $\mu_p = 0$ is both the vanishing of the purely finitely additive part
and the condition that no mass escapes to non-principal ultrafilters.  The two
descriptions are one.
...
similarity between the constructions.  CE is the condition under which finitely
additive data attains the $\sigma$-additive status needed for the observable law
to be uniquely determined; by \S\ref{sec:metatheorem}, no first-order structural
condition can play that role.

The Stone space makes the geometry of that demand visible.  Every point of
$\mathrm{St}(\mathcal{C})$ is a maximal consistent assignment of yes/no answers
to every observable distinction --- a coherent distinction pattern, whether or
not any $\omega \in \Omega$ realises it.  The observer's world sits inside
$\mathrm{St}(\mathcal{C})$ as the principal ultrafilters; beyond it lie ideal
limit points, internally coherent but unrealised.  CE is the requirement that
probability not drift there.

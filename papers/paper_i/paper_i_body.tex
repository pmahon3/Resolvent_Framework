...
A \emph{direct Carathéodory construction} assumes compatible $\sigma$-additive marginals and builds the global measure directly without topological assumptions. We then relax $\sigma$-additivity to finite additivity and regain later via extension under a support condition.

The necessary and sufficient condition for $\sigma$-additive extension is \emph{collective exhaustion} (CE): no level can assign persistent mass to events the system as a whole sees as empty.  The fact that CE is not derivable from any structural condition on the query system is precisely located by the finite-cofinite counterexample and a model-theoretic argument via \L{}o\'{s}'s theorem.

A \emph{Stone construction} accordingly assumes only finite additivity leveraging exhaustion to extend: compactness of the Stone space manufactures $\sigma$-additivity on the compact completion where CE is the support condition that descends the measure back to the space. In limit-descent exhaustion acquires geometric form via natural completion.

We take the Stone space $\mathrm{St}(\mathcal{C})$ as the  canonical compact Hausdorff completion of the observable structure: the space of all consistent distinction patterns extending the cylinder algebra.
\end{abstract}
...
Observable distinctions are classically organised by Boolean algebras, a framework whose measure-theoretic and probabilistic ramifications have been extensively developed~\cite{Stone1936, Halmos1950}.  The directed nature of observation --- wherein finer levels of resolution refine coarser ones --- has received comparatively less attention as a primary object of study.  In the foundational treatments of Kolmogorov~\cite{Kolmogorov} and Choksi~\cite{choksi1958}, directed structure appears as index scaffolding: the organisation that allows a consistent global measure to be assembled from compatible marginals, but not itself a source of probabilistic constraint.

What governs the passage from this finitely organised algebraic structure to genuine probability is neither a Boolean-algebraic fact nor a consequence of the directed structure alone.  It is a valuation-layer condition --- one that sits outside what any first-order description of the algebraic or observational structure can express~\cite{Hodges1997} --- and this paper identifies it precisely.

The condition is \emph{collective exhaustion} (CE): a compatible family of finitely additive charges extends to a $\sigma$-additive measure if and only if, whenever a decreasing sequence of events vanishes globally, there exists a level at which the sequence's mass converges to zero.  CE rules out purely finitely additive charges, which assign persistent mass to sequences that vanish in the limit.  It is the exact admissibility condition separating observational coherence from genuine probability: a coherent family of charges deserves to be treated as a probability system precisely when it satisfies CE.  This paper proves that a family satisfying CE determines a unique probability measure on the observable $\sigma$-algebra, and that no first-order structural condition can substitute for it.

The \emph{Carathéodory construction} (\S\ref{sec:cara}) is the direct local-to-global extension: given compatible $\sigma$-additive probability marginals, it builds the global observable law directly.  Theorem~\ref{thm:sp1} identifies CE as the condition under which finitely additive level data can be promoted to that starting point.

The \emph{Stone construction} (\S\ref{sec:stone}) assumes only finitely additive compatible charges and derives $\sigma$-additivity from compactness.  Descending from the Stone space to $\Omega$ requires two conditions: discriminability ensures the Stone embedding is injective, and CE appears as a support condition --- the measure concentrates on the principal ultrafilters, i.e., on the image of $\Omega$ inside its Stone compactification.

CE is a non-derivable admissibility condition: no first-order structural condition on the query system implies it, as shown by the finite-cofinite counterexample and a model-theoretic argument via \L{}o\'{s}'s theorem (\S\ref{sec:ce}).  This obstruction is not an artifact of the query-system formalism: in the more primitive first-order language of Boolean algebras with finitely additive charge, countable additivity is likewise not first-order axiomatizable --- the need for an extra admissibility condition is logically unavoidable.

The Stone space $\mathrm{St}(\mathcal{C})$ is the canonical compact Hausdorff completion of $\Omega$; CE's two faces --- algebraic convergence and geometric support --- are the same condition seen from inside and outside the algebra (\S\ref{sec:bridge}).

\paragraph{Organisation.}
Section~\ref{sec:setup} introduces query systems, cylinder sets, the observable $\sigma$-algebra, compatible charges, and the structural hypotheses (Surjective Evaluation, Discriminability).  Section~\ref{sec:cara} gives the direct extension theorem: given compatible $\sigma$-additive marginals, the global observable law follows by Carathéodory.  Section~\ref{sec:ce} establishes CE as the admissibility criterion that determines when finitely additive level data warrants that probabilistic status, proves its non-derivability from structural conditions, and shows that the underlying logical obstruction is not specific to the query-system language (\S\ref{sec:metatheorem}). Section~\ref{sec:stone} proves the Stone realization from finitely additive data. Section~\ref{sec:bridge} draws out CE's unified meaning across both constructions and characterises the Stone space.  The main results have been formalized in Lean~4 using Mathlib; source files are available in the accompanying repository.  This paper is the first step in a programme deriving canonical dynamics, state-space reconstruction, and certifiability from observational data; the further steps are carried out in the companion papers.
...
The index set $(\iota, \leq)$ admits \emph{common coarsenings} if every pair $i, j \in \iota$ has a lower bound: there exists $k \leq i$ and $k \leq j$. It admits \emph{common refinements} if every pair has an upper bound: there exists $k \geq i$ and $k \geq j$.  It admits \emph{sequential common refinements} if every countable family $\{i_n\}_{n \geq 1} \subset \iota$ has an upper bound in $\iota$.
\end{definition}

\noindent Sequential common refinements imply common refinements.  Common
coarsenings ensure cylinder sets form a $\pi$-system (Lemma~\ref{lem:pi-system}); sequential common refinements supply the $\sigma$-subadditivity step in the Carathéodory construction.
...
The two constructions impose different structural conditions on the query system.
...
realised by some $\omega \in \Omega$; it implies the refinement maps $\pi_{ij}$ are surjective and the connecting maps $\varphi_{ij}$ injective.
...
required by the Stone construction (\S\ref{sec:stone}) but not by the Carathéodory construction.
...
Theorem~\ref{thm:finite-content} gives a well-defined finitely additive content $\mu_0$ on $\mathcal{C}$.
...
\textit{Uniqueness.}
Two probability measures with the same evaluation marginals agree on $\mathcal{C}$, hence on $\sigma(\mathcal{C})$ by Theorem~\ref{thm:obs-determination}.
\end{proof}
...
Section~\ref{sec:cara} showed that compatible $\sigma$-additive marginals extend directly to a global observable law.  This section identifies the admissibility condition that determines when finitely additive level data deserves that probabilistic status.
...
not the Boolean-algebraic index structure.  The following example shows it is not automatic.
...
The sequence $E_n = \{n, n+1, \ldots\}$ satisfies $E_n \downarrow \varnothing$, yet each $E_n$ is cofinite, so $\mu_0(E_n) = 1$ for all $n$: mass is not exhausted.  No $\sigma$-additive extension exists.
\end{example}
...
within $\mathcal{E}$ alone: confirming $\bigcap_n E_n = \emptyset$ requires access to $\sigma(\mathcal{E})$.  In a directed system, one might hope a finer level $j \geq i$ could witness the emptiness level $i$ cannot see --- but the same obstruction reappears: finite additivity at $j$ does not force continuity at $\emptyset$ for sequences in $\sigma(\mathcal{E}_j)$. Section~\ref{sec:incompleteness} shows the directed-system structure alone does not provide it.
...
$i$), so $\pi_{ij}^{-1}(E_n) \subseteq O_j$ is the preimage of $E_n$ at the finer level.  No level may assign persistent mass to events the system as a whole sees as empty.
...
\begin{proof}
The particular case is Proposition~\ref{prop:independence}: the finite-cofinite content satisfies all named structural hypotheses and fails CE.
...
If $E$ is finite then $q(n) \in E$ holds for only finitely many $n$, so the ultralimit is $0$.  If $E$ is cofinite then $q(n) \in E$ holds for cofinitely many $n$, so the ultralimit is $1$, since $\mathcal{U}$ extends the cofinite filter.  The ultraproduct is therefore exactly the finite-cofinite content $\ell$ of Example~\ref{ex:finite-cofinite} on $\mathbb{Q}$.
...
finite consistency test yet assigns unit mass to events whose infinite intersection is empty.  CE is the admissibility condition under which observational coherence furnishes probability, not a structural consequence of the query system.
...
$\delta_n$ would satisfy any putative characterizing theory, yet their ultraproduct is the finite-cofinite charge $\ell$, which fails $\sigma$-additivity --- contradicting \L{}o\'{s}'s theorem.
...
The Stone construction shows that $\sigma$-additivity can instead be derived.  Starting only from finitely additive charges, compactness of the Stone space produces a $\sigma$-additive measure on the compact completion; CE then reappears as the support condition that returns this measure to the realised instantiation space.
...
\begin{proof}
Surjective evaluation and the coherence condition give $\pi_{ij}(\mathrm{eval}_j(\omega)) = \mathrm{eval}_i(\omega)$ for all $\omega$, so $\mathrm{im}(\pi_{ij}) \supseteq \mathrm{im}(\mathrm{eval}_i) = \Out{i}$: the refinement maps are surjective.  A surjective map $\pi_{ij}$ has injective preimage map $\pi_{ij}^{-1}$ on subsets, so $\varphi_{ij}(\mathrm{Cyl}(i, A)) = \mathrm{Cyl}(j, \pi_{ij}^{-1}(A))$ is injective on generators, hence on $B_i$.
\end{proof}
...
\textit{Uniqueness.}
Two probability measures on $(\Omega, \sigma(\mathcal{C}))$ agreeing on all cylinder sets are equal by Theorem~\ref{thm:obs-determination}, which requires common coarsenings.  The Stone theorem therefore assumes common coarsenings in addition to surjective evaluation, discriminability, and CE.
\end{proof}

\noindent It may appear that compactness of the Stone space eliminates the need
for CE.  Compactness ensures $\hat{\mu}$ on $\mathrm{St}(\mathcal{C})$ is $\sigma$-additive, but $\hat{\mu}$ lives on the Stone space, not on $\Omega$. Descent to $\Omega$ requires that no mass escapes to non-principal ultrafilters, and that is precisely CE (Proposition~\ref{prop:B2}).  In the Stone picture, CE is not an additional hypothesis but a \emph{support condition}: the measure concentrates on the image of $\Omega$ inside its Stone compactification.
...
The Yosida--Hewitt decomposition identifies the algebraic and geometric faces of CE: $\mu_p = 0$ is both the vanishing of the purely finitely additive part and the condition that no mass escapes to non-principal ultrafilters.  The two descriptions are one.

\begin{proposition}[Uniqueness of the observable law]
...
\begin{proof}
Both measures agree on every cylinder set: $P^{\mathrm{C}}(\mathrm{Cyl}(i, A)) = \nu_i(A) = P^{\mathrm{S}}(\mathrm{Cyl}(i, A))$.  By observational determination (Theorem~\ref{thm:obs-determination}), they are equal on $\sigma(\mathcal{C})$.
\end{proof}

\noindent The coincidence is a consequence of uniqueness, not of formal
similarity between the constructions.  CE is the condition under which finitely additive data attains the $\sigma$-additive status needed for the observable law to be uniquely determined; by \S\ref{sec:metatheorem}, no first-order structural condition can play that role.

The Stone space makes the geometry of that demand visible.  Every point of $\mathrm{St}(\mathcal{C})$ is a maximal consistent assignment of yes/no answers to every observable distinction --- a coherent distinction pattern, whether or not any $\omega \in \Omega$ realises it.  The observer's world sits inside $\mathrm{St}(\mathcal{C})$ as the principal ultrafilters; beyond it lie ideal limit points, internally coherent but unrealised.  CE is the requirement that probability not drift there.

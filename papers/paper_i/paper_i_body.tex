...
assumes $\sigma$-additive marginals and constructs the global measure directly
on the instantiation space, with no topological assumptions; CE is the
condition the marginals satisfy.
The \emph{Stone route} assumes only finite additivity: compactness of the Stone
space manufactures $\sigma$-additivity on the compact completion, and CE is then
the support condition that descends the measure back to the instantiation space.
Under shared hypotheses the two constructions coincide.

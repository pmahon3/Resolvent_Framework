...
which an observer organises experience of a system.  State space,
...
probability measure.  The further steps --- that this measure carries canonical
dynamics, that the delay structure of observation (re)constructs the state
space, and that finite data can certify (re)construction --- are carried out
in the companion papers.

What makes the first step possible is a single condition.
\emph{Collective exhaustion} (CE): a compatible family of finitely additive
charges extends to a $\sigma$-additive measure if and only if, whenever a
decreasing sequence of events vanishes globally, there exists a level at which
the sequence's mass converges to zero.  CE rules out purely finitely additive
charges, which assign persistent mass to sequences that vanish in the limit.
It is the exact admissibility condition separating observational coherence from
genuine probability: a coherent family of charges deserves to be treated as a
probability system precisely when it satisfies CE.
...
The \emph{Stone route} (\S\ref{sec:stone}) assumes only finitely additive
compatible charges and derives $\sigma$-additivity from compactness.  Stone
duality converts the directed Boolean system into an inverse system of compact
Hausdorff spaces; a compatible family of charges assembles into a
$\sigma$-additive measure on the inverse limit.  Descending from the Stone
space to $\Omega$ requires two conditions: discriminability ensures the Stone
embedding is injective, and CE appears as a support condition --- the measure
concentrates on the principal ultrafilters, i.e., on the image of $\Omega$
...
convergence and geometric support on $\Omega$ --- are the same condition seen
from inside and outside the algebra.  Its role as a completion of the state
space is developed in the companion papers.
...
Rather than a single $\sigma$-algebra of events, we work with a directed system
of Boolean algebras representing observable distinctions at increasing resolution;
the global $\sigma$-algebra arises as their completion.
...
observer can make and how finer levels coarsen.  Choosing $\Omega$ is an
additional act: it is a set of states realising the abstract structure
concretely, and coherence says evaluation is compatible with coarsening.

\subsection{Canonical instantiation and observables}

Every observational structure admits a canonical instantiation: the space of
all coherent assignments of outcomes across levels.  This identifies $\Omega$
with the set of all globally consistent observation patterns.  Explicitly, the
canonical instantiation is the projective limit
\[
  \Omega
...
coarsenings ensure cylinder sets form a $\pi$-system
(Lemma~\ref{lem:pi-system}); sequential common refinements supply the
$\sigma$-subadditivity step in the Carathéodory route.
...
\noindent Surjective evaluation ensures every outcome at every level is
realised by some $\omega \in \Omega$; it implies the refinement maps
$\pi_{ij}$ are surjective and the connecting maps $\varphi_{ij}$ injective.
...
\noindent CE is the valuation-layer condition studied in \S\ref{sec:ce}: it
rules out purely finitely additive charges and is necessary and sufficient for
$\sigma$-additive extensibility (see Example~\ref{ex:finite-cofinite}).
...
Disjoint sets in $\mathcal{E}$ are either both finite or one finite and one
cofinite (two disjoint cofinite sets cannot exist), so finite additivity holds.

The sequence $E_n = \{n, n+1, \ldots\}$ satisfies $E_n \downarrow \varnothing$,
yet each $E_n$ is cofinite, so $\mu_0(E_n) = 1$ for all $n$: mass is not
exhausted.  No $\sigma$-additive extension exists.
\end{example}
...
yields a global measure directly on the instantiation space.  CE does not appear
as a proof step; Section~\ref{sec:ce} identifies it as the admissibility
condition under which finitely additive level data can be promoted to this input.
...
By Stone's representation theorem~\cite{Stone1936}, $\mathcal{E}$ is isomorphic
to the clopen algebra of a compact totally-disconnected Hausdorff space
$S(\mathcal{E})$, and $\sigma(\mathcal{E})$ is the Baire $\sigma$-algebra of
$S(\mathcal{E})$.  The sets in $\sigma(\mathcal{E}) \setminus \mathcal{E}$ are
Baire events that countable operations can reach but no single observable
...
level $j \geq i$ could witness the emptiness level $i$ cannot see --- but the
same obstruction reappears: finite additivity at $j$ does not force continuity
at $\emptyset$ for sequences in $\sigma(\mathcal{E}_j)$.
Section~\ref{sec:incompleteness} shows the directed-system structure alone does
not provide it.
...
$i$), so $\pi_{ij}^{-1}(E_n) \subseteq O_j$ is the preimage of $E_n$ at the
finer level.  CE is a \emph{valuation-layer} condition: it constrains how
charges behave, not the Boolean-algebraic index structure --- no level may
assign persistent mass to events the system as a whole sees as empty.

The following proposition constructs the minimal directed system in which the
index-layer hypotheses hold while the finite-cofinite charge still fails
$\sigma$-additive extension.  The construction is a deliberate stress test,
not a naturally arising observational system.
...
extensions into a global measure (\S\ref{sec:cara}).

\subsection{CE is non-derivable}
\label{sec:ce-irred}

The Yosida--Hewitt decomposition~\cite{YosidaHewitt1952} identifies the
obstruction Proposition~\ref{prop:independence} has exposed.  Every finitely
additive probability decomposes uniquely as $\ell = \ell_c + \ell_p$, where
$\ell_c$ is $\sigma$-additive and $\ell_p$ is \emph{purely finitely additive}.
The finite-cofinite content has $\ell_p = \ell$.  By Theorem~\ref{thm:sp1}, CE
holds precisely when $\ell_p = 0$.  The obstruction is therefore not a feature
of any particular structural hypothesis: it is the persistence of a purely
finitely additive component that no first-order structural condition can
eliminate.  Proposition~\ref{prop:ce-irred} makes this precise.
...
\noindent The ultrafilter-based content is the canonical witness: it passes every
finite consistency test yet assigns unit mass to events whose infinite
intersection is empty.  CE is the admissibility condition under which
...
$\sigma$-additive, so any putative first-order theory characterizing
$\sigma$-additive extensibility would be satisfied by every $\delta_n$.  Their ultraproduct
...
The need for some extra admissibility principle is unavoidable.  CE is the
condition adopted here; other foundational approaches may supply different extra
structure, but some such structure is required.  Foundational disputes about
probability are not terminological; they are disputes about which extra structure
licenses the passage from finite coherence to probability.
\end{remark}
...
$\sigma$-additive measure on the compact completion; CE reappears as the
support condition that returns this measure to the realised instantiation space.

The proof proceeds in four steps.  \S\ref{sec:stepA} identifies the Stone space
of the direct limit with the inverse limit of individual Stone spaces and applies
Choksi's theorem.  \S\ref{sec:B1} identifies the pullback of the Baire algebra
of the Stone space with the observable $\sigma$-algebra.  \S\ref{sec:B2} shows
CE forces the measure to concentrate on the image of $\Omega$.
...
for CE.  Compactness ensures $\hat{\mu}$ on $\mathrm{St}(\mathcal{C})$ is
$\sigma$-additive, but $\hat{\mu}$ lives on the Stone space, not on $\Omega$.
Descent to $\Omega$ requires that no mass escapes to non-principal ultrafilters,
and that is precisely CE (Proposition~\ref{prop:B2}).  In the Stone picture, CE
is not an additional hypothesis but a \emph{support condition}: the measure
concentrates on the image of $\Omega$ inside its Stone compactification.
...
compatible probability marginals, build the global observable law directly.
The Stone route answers the admissibility question: starting from finitely
additive charges, identify CE as the condition licensing $\sigma$-additive
extension, then realise it via compactness.  That both produce the same measure
is not obvious from the constructions; it is a consequence of uniqueness
(Proposition~\ref{thm:obs-determination}).
...
from already $\sigma$-additive marginals.  Theorem~\ref{thm:sp1} identifies CE
as the algebraic condition explaining when finitely additive level charges
deserve to be treated as probability measures --- when they do not assign
persistent mass to events the refinement hierarchy sees as empty.  CE is thus
the admissibility criterion whose satisfaction licenses the Carathéodory route's
input.
...
The Yosida--Hewitt decomposition bridges the two faces: it identifies the
purely finitely additive part of $\mu$ with the mass escaped to non-principal
ultrafilters.  CE says both things at once: algebraic convergence is honest,
and the geometric measure is confined to $\Omega$.
...
criterion that explains when finitely additive data warrants that assumption.
In the Stone route, compactness manufactures $\sigma$-additivity on the compact
completion, and CE is the support condition that descends it to $\Omega$.  In
both cases CE does the same work: no mass escapes the realised observational
world.
...
route begins from local laws that already carry probabilistic status, while the
Stone route bridges it through compactness and the support condition.  What CE names, in both cases, is the same gap.

\subsection{The Stone space as compact completion}

The Stone space $\mathrm{St}(\mathcal{C})$, constructed as a technical device
for measure extension, is the canonical compact Hausdorff completion of $\Omega$
for the observable topology.  The embedding
$\mathrm{pure} : \Omega \to \mathrm{St}(\mathcal{C})$ is dense --- every
nonempty basic clopen $[E]$ meets $\mathrm{pure}(\Omega)$ --- and the Stone
space is uniquely determined by $\mathcal{C}$ up to homeomorphism: the
compactification already contained in the Boolean algebra of cylinder sets.
...
ultrafilters are ideal limit points --- consistent distinction patterns the
refinement system coheres with, but that no element of $\Omega$ realises.
The Stone space is the \emph{space of all consistent distinction patterns}: the
completion of the observer's world to a compact whole.

CE closes the loop.  It requires the measure $P$, when extended to
$\mathrm{St}(\mathcal{C})$ via $\mathrm{pure}$, to remain concentrated on the
realised part: no mass escapes to the phantom ultrafilters.  Probability remains
confined to the realised world however far the algebraic completion extends.
The Stone topology, generated by all cylinder clopens, is the coarsest topology
making every observable distinction continuous.

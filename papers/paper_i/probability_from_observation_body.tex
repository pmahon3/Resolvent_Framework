...
the query system --- a concrete counterexample shows the particular case,
and an ultraproduct argument via {\L}o\'s's theorem settles the universal
case --- and the same obstruction already appears in the first-order
language of Boolean algebras with finitely additive charge, where
countable additivity is likewise not first-order axiomatizable.  Some extra
...
likewise not first-order axiomatizable~\cite{MahonCE2026}.  The
probability-logic tradition --- Hoover~\cite{Hoover1978} and Fagin, Halpern,
and Megiddo~\cite{FaginHalpernMegiddo1990} --- treated this as a background
fact; the companion note records a self-contained proof.
...
\textit{Step 2.}
Let $\delta_n$ be the Dirac probability at $n \in \mathbb{N}$ on $\mathcal{P}(\mathbb{N})$, and let $\mathcal{U}$ be a nonprincipal ultrafilter on $\mathbb{N}$.  Each $\delta_n$ is a normalized $\sigma$-additive charge satisfying every structural condition including CE.  The charge induced on the diagonal copy of $\mathcal{P}(\mathbb{N})$ in the ultraproduct $\prod_{\mathcal{U}} (\mathcal{P}(\mathbb{N}), \delta_n)$ is
\[
  \ell(A) \;=\; \lim_{\mathcal{U}} \mathbf{1}_{n \in A},
\]
the $\{0,1\}$-valued ultrafilter charge: $\ell(A)=1$ iff $A \in \mathcal{U}$.  Since $\mathcal{U}$ is nonprincipal, $\ell(\{n\})=0$ for every $n$, yet $\ell(\mathbb{N})=1$.  The sets $A_k = \{k, k{+}1, \ldots\}$ satisfy $A_k \downarrow \varnothing$ and $\ell(A_k)=1$ for all $k$, so $\ell$ fails CE.

\textit{Step 3.}
By Step~1, any universally valid first-order condition $\Phi$ holds in the ultraproduct whenever it holds in each $\delta_n$.  But by Step~2 the charge on the diagonal copy fails CE.  Therefore no first-order structural condition can imply CE.
\end{proof}
...
The obstruction in Proposition~\ref{I:prop:ce-irred} is not an artifact of the query-system language.  In the more primitive setting of Boolean algebras with a normalized finitely additive charge, the same conclusion holds: no first-order theory in that language characterizes the models in which the charge is $\sigma$-additive.  In particular, countable additivity is not first-order axiomatizable among finitely additive probability Boolean algebras \cite{MahonCE2026}.  The probability logic community has treated this as background knowledge --- Hoover~\cite{Hoover1978} requires an infinitary rule to axiomatize $\sigma$-additive probability logic, and Fagin, Halpern, and Megiddo~\cite{FaginHalpernMegiddo1990} state without proof that countable additivity cannot be expressed by any formula in the natural language.  The companion note \cite{MahonCE2026} records a self-contained ultraproduct proof: Dirac masses $\delta_n$ on $\mathcal{P}(\mathbb{N})$ are individually $\sigma$-additive, but the charge induced on the diagonal copy of $\mathcal{P}(\mathbb{N})$ in their ultraproduct is the $\{0,1\}$-valued ultrafilter charge, which is not $\sigma$-additive --- contradicting \L{}o\'{s}'s theorem if any first-order axiomatization existed.

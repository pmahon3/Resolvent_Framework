...
When some observations preclude others, the observation algebra
loses distributivity.
...
A state $s$ on $A$ defines $\mu(h(a)) = s(a)$ on the clopen
$\perp$-stable sets $\CO(S_0(A))^\dagger$.
...
The preceding analysis reveals a filtration of coherence conditions
on an observation algebra~$A$.  The conditions are linearly ordered
by implication, but the three transitions between them are of
different mathematical character --- each requires a different key.

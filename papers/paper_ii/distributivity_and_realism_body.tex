...
\smallskip
The dividing line is distributivity.  The individual ingredients ---
...
For OMLs of the form $L(H)$ with $\dim H \geq 3$, distributivity
is the exact condition under which value-definite realism and
empirical adequacy are commensurable.
\end{corollary}

\begin{proof}
If $A$ is distributive, VDR holds
(Theorem~\ref{thm:commensurability}(a)).  If $A \cong L(H)$ with
$\dim H \geq 3$, then $A$ is non-distributive and VDR fails
...
The underlying structure is a filtration of the state space, whose
three transitions require different keys.
...
Einstein's realism (``God does not play dice'') is the demand for
VDR; the Kochen--Specker theorem shows it is algebraically
impossible, not merely philosophically contestable.  Bohr's
complementarity is the non-triviality of the pasting transition.
...
\begin{enumerate}
  ...
  \item \emph{The topos connection.}
    The topos approach to quantum
    theory~\cite{IshamButterfield1998, DoeringIsham2008,
    HeunenLandsmanSpitters2009} addresses closely related questions
    via spectral presheaves rather than the McDonald--Bimb\'{o}
    filter-space duality.  In particular, the Bohrification
    programme of Heunen, Landsman, and
    Spitters~\cite{HeunenLandsmanSpitters2009} constructs a topos
    from the poset of Boolean subalgebras --- the same contexts
    that appear in our filtration.  A comparison of the two
    frameworks --- and whether the commensurability theorem has a

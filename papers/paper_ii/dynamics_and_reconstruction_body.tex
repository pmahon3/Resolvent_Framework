...
\begin{abstract}
Starting from a $\sigma$-additive probability measure on the algebra of discriminable
events~\cite{mahon_paper1}, the Rokhlin disintegration theorem forces a conditional
regularity kernel and a minimal sufficient factor $Q_*$, the coarsest reduction
carrying full conditional information.  Indexing over time, temporal coherence forces
the Chapman--Kolmogorov equation, yielding a Markov semigroup $\{\Pi_t\}$ and
Koopman--Perron duality; within this observational formalism, no dynamical law
is separately posited --- once conditional regularity is indexed coherently
over time, the semigroup and Koopman operator follow from the measure.  In a measure-preserving system $Q_*$ becomes the delay map
$\Phi_h$, and reconstruction --- that $\mathcal{O}_h = \mathcal{B}$ mod $\mu$, density
of the delay algebra in $L^2$, and injectivity of $\Phi_h$ are equivalent --- is the
faithfulness question about this factor.  The Stone space~\cite{mahon_paper1} built to
derive $\sigma$-additivity is identified with $X$ itself, up to measure-theoretic
...
disintegration theorem~\cite{Rokhlin1952} forces into existence — it need not be separately posited.  Composing
...
about regularity of $S$; nor do the finite-lag extensions of
...
embedding framework.  The reconstruction theorem here differs in kind: it
requires only measurability, uses all delays simultaneously, and replaces the
dimension count with the algebraic density condition; in exchange, the
embedding is measure-theoretic rather than topological.  On a compact manifold
...
The measure derived in~\cite{mahon_paper1} already contains, without
further assumption, the structures developed here.  The Rokhlin
disintegration reads off a conditional regularity kernel for any pair
...
The mathematical content of §§\ref{II:sec:prediction}--\ref{II:sec:stone} is standard once a $\sigma$-additive measure is in hand on the observable algebra; what the programme changes is the explanatory position of each object.  The individual components are classical.  Rokhlin disintegration~\cite{Rokhlin1952}
...
The Stone space $\mathrm{St}(\mathcal{C})$ built there to establish
...
affirmatively, under faithfulness.

What faithfulness means in practice, and whether it can be certified
from finite data, is taken up in~\cite{mahon_paper3}.

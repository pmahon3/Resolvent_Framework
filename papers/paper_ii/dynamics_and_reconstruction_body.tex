...
To study a dynamical system through its observables, Koopman~\cite{Koopman1931}
begins from the transformation $T$ and defines an operator: $f \mapsto f \circ T$.
To study prediction, the stochastic-process tradition begins from a filtration
and builds conditional expectations within it~\cite{Doob1953}.  In both cases
the dynamical or temporal structure is written in first, and the probability
measure is defined on top of it.  This is the natural order.

But the probability measure, once in hand, already contains more than it was
given credit for.  Any two observations $Q$ and $F$ — no hypothesis on time or
order — determine a conditional regularity of $F$ given $Q$: the Markov kernel
$\kappa_Q(q,\cdot) = P(F \in \cdot \mid Q = q)$, which the Rokhlin
disintegration theorem~\cite{Rokhlin1952} forces into existence once
$\sigma$-additivity holds.  It is not added; it is already there.  Composing

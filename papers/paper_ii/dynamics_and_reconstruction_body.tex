...
Before specialising to the delay-map setting, it is worth noting the general
form of the reconstruction question and the role CE plays in it.  Given any
...
for any decreasing sequence $A_k \in \sigma(\mathcal{C})$ with $\bigcap_k A_k$
a $\mu$-null set, $\mu(A_k) \to 0$.  The events in $\sigma(\mathcal{C})$
not approximable by $\mathcal{G}_k$ form a decreasing sequence whose
intersection is empty as the refinement exhausts the system.
By CE, their $\mu$-measure tends to zero, so $\delta(\mathcal{G}_k) \to 0$.
\end{proof}
...
is an honest refinement scheme (increasing lag is automatically exhaustive when
$\mathcal{O}_h = \mathcal{B}$ mod $\mu$).  The separation defect $\delta(\mathcal{G})$

...
The standard approaches to dynamics and prediction each commit to temporal
structure before probability is fully in hand.  Koopman's
approach~\cite{Koopman1931} begins from a transformation $T$ and studies how
it acts on observables; the dynamical system is primitive, and the operator
$f \mapsto f \circ T$ is derived.  The stochastic-process tradition begins
from a filtration — time-indexed $\sigma$-algebras carrying what is known up
to each moment — and builds prediction within that ordered
structure~\cite{Doob1953}.  Both take order as given.

The present paper begins earlier.  Any two observations $Q$ and $F$ of a
probability space, with no hypothesis on time or order, already determine a
conditional regularity of $F$ given $Q$: a Markov kernel
$\kappa_Q(q, \cdot) = P(F \in \cdot \mid Q = q)$, forced by the Rokhlin
disintegration theorem~\cite{Rokhlin1952} once $\sigma$-additivity is in hand.
It is not a modelling choice.  Composing with $Q$ gives the minimal sufficient
factor $Q_*$, the coarsest reduction of the sample point carrying full
conditional information about $F$.  Temporal prediction is the special case in
which $F$ is a future observable; the object itself is neutral on time.

When the conditional regularity is indexed over time, the kernels $\{\Pi_t\}$
must compose consistently.  That coherence forces the Chapman--Kolmogorov
equation — it is derived, not assumed — yielding a Markov semigroup and
Koopman--Perron duality $\int K_t g\, d\mu = \int g\, d(\mu \star \Pi_t)$.
...
then internal: does this factor see all of $X$, or only a proper quotient?
The answer is a three-way equivalence — $\mathcal{O}_h = \mathcal{B}$ mod
$\mu$, density of the delay algebra in $L^2$, and injectivity of $\Phi_h$ as a
measure-theoretic embedding — proved via a density bridge between
$\sigma$-algebra generation and $L^2$ closure.  The Stone
space~\cite{mahon_paper1} built to derive $\sigma$-additivity is identified
with $X$ itself when reconstruction holds.

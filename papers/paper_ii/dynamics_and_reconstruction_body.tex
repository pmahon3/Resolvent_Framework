...
natural, but it need not be conceptually prior.  One may instead ask whether an
observable probability measure already contains a conditional regularity of one
query relative to another — and whether the familiar semigroup and predictive
structures emerge only when such regularities are indexed coherently over time.

It does.  Any two observations $Q$ and $F$ — no hypothesis on time or
order — already determine a conditional regularity of $F$ given $Q$: the Markov kernel

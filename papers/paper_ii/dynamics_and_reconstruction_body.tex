...
When reconstruction holds, the Stone space~\cite{mahon_paper1}
$\mathrm{St}(\mathcal{O}_h)$ — built as a compact completion of the sample
space for deriving $\sigma$-additivity — turns out to be measure-theoretically
isomorphic to $X$ itself.  The programme that began by seeking a compact ambient space for the
...
\subsection{Minimal predictive state}
...
The predictive kernel is $\Pi(i,\cdot) = P(i,\cdot)$, the $i$-th row of $P$:
this is the unique Markov kernel satisfying
...
Before specialising to the delay-map setting, the general form of the
reconstruction question and the role CE plays in it deserve to be named.  Given any
finite family of queried levels $n_1, \ldots, n_k$ from a directed query system
$(\mathcal{E}_n)_n$ with realization measure $\mu$ on $\Omega$, define the
\emph{separation defect} of the joined algebra
$\mathcal{G} = \mathcal{E}_{n_1} \vee \cdots \vee \mathcal{E}_{n_k}$ by
\[
  \delta(\mathcal{G})
  \;:=\;
  \sup_{A \in \sigma(\mathcal{C})} \inf_{E \in \mathcal{G}} \mu(A \triangle E),
\]
where $\sigma(\mathcal{C})$ is the observable $\sigma$-algebra generated by the
full query system.  This quantity measures how much of the observable structure
remains invisible to the finite queried subfamily: $\delta(\mathcal{G}) = 0$ if
and only if $\sigma(\mathcal{G}) = \sigma(\mathcal{C})$ modulo $\mu$.

A sequence $(\mathcal{G}_k)$ of joined algebras is \emph{honest} if it is
increasing and exhaustive:
$\sigma(\bigcup_k \mathcal{G}_k) = \sigma(\mathcal{C})$ modulo $\mu$.  Under
an honest refinement scheme, the separation defect is non-increasing in $k$.

\begin{proposition}[CE drives separation defect to zero]
\label{II:prop:ce-sep-defect}
...
\begin{proof}
Fix $\varepsilon > 0$ and $A \in \sigma(\mathcal{C})$.  Since $(\mathcal{G}_k)$
is honest, $A$ is approximable modulo $\mu$ by some $E_k \in \mathcal{G}_k$
for all large $k$: the sets $A \triangle E_k$ decrease to a $\mu$-null set.
CE~\cite{mahon_paper1} rules out persistent mass on globally vanishing events,
so $\mu(A \triangle E_k) \to 0$.  Taking the supremum over $A$ and the
infimum over $E_k$ gives $\delta(\mathcal{G}_k) \to 0$.
\end{proof}

\begin{remark}[The temporal specialisation]
\label{II:rem:temporal-spec}
In the measure-preserving setting with $Q_t = h \circ T^t$, the finite
joined algebra $\mathcal{O}_h^{(L)} = \sigma(h, h \circ T, \ldots, h \circ T^L)$
forms an increasing family, and under reconstruction ($\mathcal{O}_h = \mathcal{B}$
mod $\mu$, established in §\ref{II:sec:density-bridge}--\ref{II:sec:stone}) it
is exhaustive — hence an honest refinement scheme.  The separation defect $\delta(\mathcal{G})$
specialises to $\delta(L) := \delta(\mathcal{O}_h^{(L)})$, the quantity
that drives the finite-sample theory of~\cite{mahon_paper3}.  There,
exponential decay of $\delta(L)$ under mixing yields minimax-optimal
certification rates; the honest-refinement condition is automatic because
lag growth is the canonical exhaustion mechanism.  In the general query
setting, these two roles — exhaustion and mixing — must be supplied
separately.
\end{remark}

...
\begin{abstract}
Starting from a $\sigma$-additive probability measure on an observable algebra —
the object constructed in~\cite{mahon_paper1} from coherent structured observations
satisfying collective exhaustion — this paper derives two structures that are often
...
This raises the reconstruction question: when does $\Phi_h$ faithfully represent
...
When reconstruction holds, the compact Stone space of~\cite{mahon_paper1} —
introduced there as a technical device for $\sigma$-additivity — is identified
with $X$ itself.
\end{abstract}
...
measure on the observable algebra.  That measure is not an extra assumption.  It is
what~\cite{mahon_paper1} constructs: a coherent family of finitely additive charges
on a directed system of observable algebras extends to a genuine probability measure
exactly when it satisfies collective exhaustion, the condition ruling out persistent
mass on globally vanishing events.
...
$\Phi_h(x) = (h(T^n x))_{n \in \mathbb{Z}}$.  This raises the second question:
when does $\Phi_h$ faithfully represent the full state space $X$?

The second question is the reconstruction problem.  Whether $\Phi_h$ is injective
modulo null sets — whether the predictive state space faithfully represents $X$ —
depends on how much the delay orbit $\{h \circ T^n : n \in \mathbb{Z}\}$
collectively distinguishes points of $X$.  The reconstruction theorem gives a
three-way equivalence: the $\sigma$-algebraic condition
$\mathcal{O}_h = \mathcal{B}$ modulo $\mu$, the analytic condition that the delay
algebra $\mathcal{A}_h$ is $L^2$-dense, and the geometric condition that $\Phi_h$
is a measure-theoretic embedding are all equivalent.
...
proof goes via the functional monotone class theorem~\cite{Dynkin}.  The
$\sigma$-algebra and the $L^2$ space it generates are inextricably linked.

The Koopman operator $U_T f = f \circ T$ provides a spectral perspective on the
second question.  The cyclic vector condition —
$\overline{\mathrm{span}}\{U_T^n h : n \in \mathbb{Z}\} = L^2$ — implies
reconstruction but is strictly stronger: it asks for density of the linear span,
while reconstruction asks only for density of the algebra the orbit generates.
The two coincide precisely when $U_T$ has simple $L^2$-spectrum.
...
The paper is self-contained modulo the probability measure.  Sections~\ref{sec:prediction}
and~\ref{sec:dynamics} develop the predictive structure from a probability space
$(\Omega, \mathcal{F}, P)$.  Section~\ref{sec:transition} connects the two parts:
the observable dynamical system produced by prediction is the natural input to the
reconstruction problem.  Sections~\ref{sec:density-bridge} and~\ref{sec:stone} carry
out the reconstruction, proving the density bridge and the reconstruction theorem.
...
The Koopman--Perron duality (Theorem~\ref{thm:koopman-perron}) connects this to a
...
Corollary~\ref{cor:cyclic-implies-reconstruction}.
\end{remark}

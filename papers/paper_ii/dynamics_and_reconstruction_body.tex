...
compatible.  For non-smooth or zero-dimensional systems — the Cantor example
...
The individual components are classical.  Rokhlin disintegration~\cite{Rokhlin1952}
...
state through observable predictions.  What is not classical is the order of assembly:
here the kernel, the factor, the semigroup, and the delay map are not assumed, chosen,
or built from a prior dynamical law --- they are consequences of a single measure
on an observable algebra, disclosed in sequence.  The novelty is architectural, not
component-wise.

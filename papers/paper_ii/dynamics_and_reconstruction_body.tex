...
and the recoverability of the underlying state space.  The Rokhlin disintegration
theorem forces a conditional regularity kernel $\kappa_Q$ into existence once
$\sigma$-additivity holds; composing with $Q$ gives the minimal sufficient factor
$Q_*$, the coarsest reduction carrying full conditional information.  Indexing
over time, temporal coherence forces the Chapman--Kolmogorov equation — derived,
not assumed — yielding a Markov semigroup $\{\Pi_t\}$ and Koopman--Perron duality.
...
Reconstruction is an exact asymptotic condition.  It answers the faithfulness
question at the limit — all lags, infinite data — but is silent on what a
finite time series can certify.  An observer with $n$ data points can estimate
the separation defect $\delta(L)$ at any fixed lag $L$, but cannot directly
witness whether $\delta(L) \to 0$ as $L \to \infty$.  The gap between
the asymptotic equivalence established here and what finite data can honestly
certify is not a limitation of the theorem; it is the next question.

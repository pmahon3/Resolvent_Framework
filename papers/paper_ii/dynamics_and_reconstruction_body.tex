...
$\Pi_Q : \alpha \to \mathcal{P}(\beta)$ satisfying
$P(F \in \cdot \mid Q = q) = \Pi_Q(q, \cdot)$ simultaneously across all outcomes.
...
$\Pi_Q : \alpha \to \mathcal{P}(\beta)$ satisfying
$P(F \in \cdot \mid Q = q) = \Pi_Q(q, \cdot)$ simultaneously across all
...
$Q_* : \Omega \to \mathcal{P}(\beta)$, $Q_*(\omega) = \Pi_Q(Q(\omega), \cdot)$:
...
The \emph{conditional regularity kernel} of $F$ given $Q$ is $\Pi_Q$, the
...
When the conditional regularity kernel $\Pi_Q$ is indexed over a time parameter,

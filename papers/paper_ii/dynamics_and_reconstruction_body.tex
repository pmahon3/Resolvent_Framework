...
algebra~\cite{mahon_paper1}, this paper derives two structures that are
standardly assumed rather than earned: the dynamics of observation under time,
and the recoverability of the underlying state space.  Both are already present
in the probability measure; what is required is to read them off.

Two observations $Q$ and $F$, together with $P$, determine a conditional
regularity of $F$ relative to $Q$: the unique Markov kernel
$\\kappa_Q : \alpha \to \mathcal{P}(\beta)$ satisfying
$P(F \in \cdot \mid Q = q) = \\kappa_Q(q, \cdot)$ simultaneously across all outcomes.
Its existence is forced by the Rokhlin disintegration theorem~\cite{Rokhlin1952}
once $\sigma$-additivity is in hand; it is not a modelling choice.
Composing with $Q$ gives the \emph{minimal sufficient factor}
$Q_* : \Omega \to \mathcal{P}(\beta)$, the coarsest reduction carrying the
full conditional regularity.  Temporal prediction is the special case in which
$F$ is a future observable; the object itself is neutral on time.

We write $\kappa_Q$ for the conditional regularity kernel of $F$ given $Q$,
reserving $\Pi_t$ for the time-indexed semigroup kernels.
Indexing over time, the kernels $\{\Pi_t\}$ must compose consistently.
Temporal coherence of the conditional regularity forces the Chapman--Kolmogorov
equation — derived, not assumed — yielding a Markov semigroup $\{K_t\}$ and the
Koopman--Perron duality $\int K_t g\, d\mu = \int g\, d(\mu \star \Pi_t)$.
When the kernels are Dirac measures, $K_t$ collapses to the classical Koopman
operator~\cite{Koopman1931}.

In a measure-preserving system $(X, \mathcal{B}, \mu, T)$ with $Q_t = h \circ T^t$,
the minimal sufficient factor $Q_*$ becomes the delay map
$\Phi_h(x) = (h(T^n x))_{n \in \mathbb{Z}}$.
The reconstruction question is then internal to the same structure: whether this
factor is faithful — whether it sees all of $X$ or only a proper quotient.
...
measure-theoretic embedding.  The key is the density bridge lemma: for any
subalgebra $\mathcal{A} \subseteq L^\infty$ containing $1$, the $L^2$-closure of
$\mathcal{A}$ equals $L^2(X, \sigma(\mathcal{A}), \mu)$.  The compact Stone
space~\cite{mahon_paper1} — built as a technical device for deriving
...
algebra~\cite{mahon_paper1}, this paper derives two structures that are
standardly assumed rather than earned: the dynamics of observation under time,
and the recoverability of the underlying state space.  Both turn out to be
already present in the probability measure; what is required is to read them
off.  The compact Stone space built in~\cite{mahon_paper1} as a technical
device for deriving $\sigma$-additivity reappears at the paper's close as the
state space itself — the programme that began by constructing an ambient space
for the measure ends by recognising it as the object the observer was
always trying to see.

Two observations $Q : \Omega \to \alpha$ and $F : \Omega \to \beta$, together
with a probability measure $P$ on the observable algebra, already determine a
conditional regularity of $F$ relative to $Q$: the unique Markov kernel
$\\kappa_Q : \alpha \to \mathcal{P}(\beta)$ satisfying
$P(F \in \cdot \mid Q = q) = \\kappa_Q(q, \cdot)$ simultaneously across all
outcomes.  The kernel is not a modelling choice; its existence is forced by the
Rokhlin disintegration theorem~\cite{Rokhlin1952} once $\sigma$-additivity is
in hand.  Composing with $Q$ gives the minimal sufficient factor
$Q_* : \Omega \to \mathcal{P}(\beta)$, $Q_*(\omega) = \\kappa_Q(Q(\omega), \cdot)$:
the coarsest reduction of $\omega$ that already carries the full conditional
regularity of $F$ given $Q$.  Temporal prediction is the special case in which
$F$ is a future observable; the object itself is neutral on time.

We write $\kappa_Q$ for the conditional regularity kernel of $F$ given $Q$,
reserving $\Pi_t$ for the time-indexed semigroup kernels.
Indexing over a time parameter, the kernels $\{\Pi_t\}$ must compose
consistently.  Temporal coherence of the conditional regularity forces the
Chapman--Kolmogorov equation — it is not assumed but derived — from which
emerge a Markov semigroup $\{K_t\}$ and the Koopman--Perron duality
$\int K_t g\, d\mu = \int g\, d(\mu \star \Pi_t)$.  When the kernels are
Dirac measures, $K_t$ collapses to the classical Koopman
operator~\cite{Koopman1931}.

The resulting structure is a canonical triple
$(\mathcal{P}(\beta), \nu_{Q_*}, \{K_t\})$.
In a measure-preserving system $(X, \mathcal{B}, \mu, T)$ with
$Q_t = h \circ T^t$, the factor $Q_*$ specialises to the delay map
$\Phi_h(x) = (h(T^n x))_{n \in \mathbb{Z}}$, encoding the dynamical
information that $h$ can distinguish.  The question of faithfulness — whether
this factor sees all of $X$ or only a proper quotient — is then internal to
the same structure.  The answer is a three-way equivalence: the observable
$\sigma$-algebra condition $\mathcal{O}_h = \mathcal{B}$ modulo $\mu$, the
analytic condition that the delay algebra $\mathcal{A}_h$ is $L^2$-dense, and
the geometric condition that $\Phi_h$ is a measure-theoretic embedding are all
equivalent.
...
$\mathcal{A}$ equals $L^2(X, \sigma(\mathcal{A}), \mu)$, proved via the
functional monotone class theorem~\cite{Dynkin}.  The $\sigma$-algebraic and
analytic conditions are not two separate constraints but two expressions of the
same one.

The paper is self-contained modulo the probability measure.
Sections~\ref{II:sec:prediction} and~\ref{II:sec:dynamics} derive the
conditional regularity kernel and the semigroup structure from a probability
space $(\Omega, \mathcal{F}, P)$.
Section~\ref{II:sec:transition} specialises to measure-preserving systems,
where $Q_*$ becomes the delay map and faithfulness of the factor becomes the
reconstruction question.
Sections~\ref{II:sec:density-bridge} and~\ref{II:sec:stone} prove the density
bridge and reconstruction theorem, and identify the Stone space with $X$.
...
Two observations $Q$ and $F$, together with $P$, already determine a conditional
regularity of $F$ relative to $Q$: a systematic, outcome-by-outcome account of how
$F$ is distributed given each value of $Q$.  The regular conditional distribution
of $F$ given $Q$ is the unique (up to $P \circ Q^{-1}$-null sets) Markov kernel
$\\kappa_Q : \alpha \to \mathcal{P}(\beta)$ satisfying
\[
...
$\Pi(i,\cdot) = P(i,\cdot)$, the $i$-th row of $P$: this is the unique Markov
kernel satisfying
$\mathbb{E}_\mu[f(X_1) \mid X_0 = i] = \sum_j p_{ij} f(j)$ for all bounded $f$.
The $t$-step kernel is $\Pi^{(t)}(i,\cdot) = P^t(i,\cdot)$, and temporal
coherence gives $\Pi^{(t+s)} = \Pi^{(t)} \circ_\kappa \Pi^{(s)}$, i.e.\
$P^{t+s} = P^t P^s$.  The semigroup acts as
\[

...
time series.
Both results of~\cite{mahon_paper1,mahon_paper2} are taken as given;
...
Given $N$ independent samples of the joint query response at levels
$(n_1, \ldots, n_k)$, define the \emph{empirical separation defect}
\[
  \hat\delta(\mathcal{G}, N)
  \;:=\;
  \frac{1}{N^2} \sum_{i,j} \mathbf{1}[\text{responses } i \text{ and } j
  \text{ agree on all of } \mathcal{G}],
\]
the empirical collision probability of the joint response vector.  This is
a U-statistic requiring no geometric structure on the realization space.
...
The remainder of this paper works in the dynamical specialisation — the setting
where rates are achievable.  Setting $Q_t = h \circ T^t$ in a
measure-preserving system $(X, \mathcal{B}, \mu, T)$, the joined algebra
$\mathcal{G} = \mathcal{O}_h^{(L)}$ is automatically an honest refinement
scheme (increasing lag exhausts $\sigma(\mathcal{C}) = \mathcal{O}_h$ under
reconstruction), and the population separation defect specialises to
$\delta(L) := \delta(\mathcal{O}_h^{(L)})$.  Exponential decay of $\delta(L)$
under mixing then yields the minimax-optimal certification rates of
Theorems~\ref{III:thm:algebra}--\ref{III:thm:conjunction}.

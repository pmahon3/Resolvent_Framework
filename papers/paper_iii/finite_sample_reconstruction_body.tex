...
\begin{abstract}
Papers~\cite{mahon_paper1,mahon_paper2} establish that a $\sigma$-additive
probability measure is forced by observational coherence (collective
exhaustion), and that this measure already contains, without further
assumption, both the conditional regularity structure and the dynamical
semigroup — with reconstruction as the question of whether the minimal
sufficient factor is faithful.  This paper asks what finite data can certify
about those structures.

At the level of a general query system, the \emph{population separation
defect} $\delta(\mathcal{G})$ measures how far finite observations fall from
full resolution; it vanishes under collective exhaustion and honest refinement.
The empirical collision probability is a U-statistic proxy, and a persistent
empirical floor is evidence against collective exhaustion — a CE-shadow theorem
requiring no temporal or geometric structure.

In the dynamical specialisation, the certification framework meets the
reconstruction theorem of~\cite{mahon_paper2}: honest refinement, a design
condition in the general setting, is here automatic — a consequence of
reconstruction — and the remaining degrees of freedom (mixing rate, embedding
dimension) are what rate-optimal guarantees exploit.  Three theorems certify
reconstruction from a time series $x_1,\ldots,x_n$.  The \emph{Algebra
Theorem} shows that the elbow stopping rule achieves the minimax-optimal rate
$n^{-s/(2s+d)}$ without oracle knowledge of the mixing rate, lag, or smoothness
index.  The \emph{Dynamics Theorem} shows that under uniform separation,
empirical delay vectors constitute a geometric witness converging at rate
$n^{-\beta/(2\beta+d)}$.  The \emph{Conjunction Theorem} shows that for
deterministic $T$, the algebraic and geometric witnesses certify the same
event — not merely correlated, but identical.

All three witnesses, together with a fourth information-theoretic one from the
collision entropy of the delay-vector distribution, measure the same failure:
separation collapsing across delay fibres.  Under fibre mixing, they are
equivalent.  The convergence of three independently motivated certifications
onto a single geometric object is the main message.
\end{abstract}
...
A probability measure on an observable algebra already contains, without
further assumption, a conditional regularity structure --- the systematic,
outcome-by-outcome account of how one observation is distributed given
another~\cite{mahon_paper2}.  Concretely, the Rokhlin disintegration of
any two observations $Q$, $F$ gives a Markov kernel
$\kappa_Q(q, \cdot) = P(F \in \cdot \mid Q = q)$ that is forced by the
measure, not chosen.
Composing with $Q$ gives the minimal sufficient factor $Q_*$, the coarsest
reduction of the sample point carrying full conditional information about $F$.
In a measure-preserving system with $Q_t = h \circ T^t$, this factor becomes
the delay map $\Phi_h$, and the reconstruction question becomes internal: is
the factor faithful, or does it see only a proper quotient of $X$?
The reconstruction theorem of~\cite{mahon_paper2} answers in terms of an
observable algebra condition — $\mathcal{O}_h = \sigma\{h \circ T^n : n \in
\mathbb{Z}\}$ generates $\mathcal{B}$ modulo $\mu$ if and only if $\Phi_h$
is a measure-theoretic embedding.
This is an exact asymptotic result; it says nothing about rates, certificates,
or what can be inferred from a finite time series.
The results of~\cite{mahon_paper1,mahon_paper2} are taken as given; this paper
asks what finite data can certify about the structure they establish.

The certification question has two natural levels, and the paper works from
the more general inward.  At the level of a general query
system~\cite{mahon_paper1} --- before any dynamical or geometric structure is
assumed --- one can ask: given finite responses to a directed family of
queries, does the empirical evidence certify that the observable structure is
resolving correctly, and can CE-failure be detected from data?
Section~\ref{III:sec:general} answers this at full generality.
The central objects are the population separation defect $\delta(\mathcal{G})$
and its empirical U-statistic proxy; the main results are a population
convergence theorem (CE drives the defect to zero under honest refinement),
an estimation theorem (the empirical proxy concentrates under IID sampling),
and a CE-shadow corollary (a persistent empirical floor is evidence against
CE).  The corollary has no temporal hypothesis; the admissibility condition CE
acquires an empirical signature that is entirely independent of dynamics.

Moving to the dynamical specialisation, the two threads of the programme
converge.  Section~\ref{III:sec:general}'s certification framework meets the
reconstruction theorem of~\cite{mahon_paper2}, and the meeting point is
stronger than either alone.  In the general query setting, honest refinement
is a design condition: it must be built into the query family.  In the
dynamical setting, it is automatic --- a consequence of the reconstruction
theorem itself, which guarantees that $\sigma(\bigcup_L \mathcal{O}_h^{(L)})
= \mathcal{B}$ modulo $\mu$ once the factor is faithful.  The observer does
not need to design an honest query path; the dynamics provide it.  The
remaining degrees of freedom --- mixing rate and embedding dimension --- are
what the rate theorems exploit.  The defect specialises to
$\deltaL = \delta(\mathcal{O}_h^{(L)})$.  Three theorems certify
reconstruction in this regime.
...
The discussion section records where each proof requires mixing and what
would be needed to extend beyond it.
...
Fix a query system $(\mathcal{E}_n)_n$ with observable $\sigma$-algebra
$\sigma(\mathcal{C})$ and realization measure $\mu$ satisfying
CE~\cite{mahon_paper1}.  For a finite joined query algebra
$\mathcal{G} = \mathcal{E}_{n_1} \vee \cdots \vee \mathcal{E}_{n_k}$,
the \emph{population separation defect} is
\[
  \delta(\mathcal{G})
  \;:=\;
  \sup_{A \in \sigma(\mathcal{C})} \inf_{E \in \mathcal{G}} \mu(A \triangle E).
\]
It vanishes iff $\sigma(\mathcal{G}) = \sigma(\mathcal{C})$ mod $\mu$.
A scheme $(\mathcal{G}_k)$ is \emph{honest} if it is increasing and
$\sigma(\bigcup_k \mathcal{G}_k) = \sigma(\mathcal{C})$ mod $\mu$; under
an honest scheme $\delta(\mathcal{G}_k)$ is non-increasing.
...
\begin{proof}
Since $(\mathcal{G}_k)$ is honest, every $A \in \sigma(\mathcal{C})$ is
$\sigma(\bigcup_k \mathcal{G}_k)$-measurable, so the martingale
$\mathbb{E}_\mu[\mathbf{1}_A \mid \mathcal{G}_k]$ converges to
$\mathbf{1}_A$ in $L^1(\mu)$.  Thus $\inf_{E \in \mathcal{G}_k}
\mu(A \triangle E) \to 0$ for each $A$.  CE~\cite{mahon_paper1} ensures
this convergence is uniform: no mass persists on the residual sets.
Hence $\delta(\mathcal{G}_k) \to 0$.
\end{proof}
...
Under an honest refinement scheme and IID sampling, if
$\hat\delta(\mathcal{G}_k, N) \geq c > 0$ for all large $k$ and $N$,
that is evidence against CE: CE is what guarantees the floor can be
discharged by further refinement.
\end{corollary}

\begin{remark}[Scope]
\label{III:rem:ce-shadow-scope}
The corollary is one-directional: persistent floor implies CE-failure;
$\delta(\mathcal{G}_k) \to 0$ does not imply CE.  The defect is defined
inside $\sigma(\mathcal{C})$, while CE also controls escape to the Stone
boundary — non-principal ultrafilters invisible to the defect functional.
A charge family can resolve $\sigma(\mathcal{C})$ completely while still
placing mass on non-realised boundary points~\cite{mahon_paper1}.

Honest refinement is essential: a stalling empirical defect under a
poorly-chosen query path is a design artifact, not an admissibility
failure.  Under an honest scheme it is the latter.

CE thereby acquires an empirical signature absent from its abstract
treatment in~\cite{mahon_paper1}: it is not only the admissibility
condition for $\sigma$-additivity (Theorem~I:thm:sp1 of~\cite{mahon_paper1}),
but a structural hypothesis with a detectable finite-sample consequence.
\end{remark}

\begin{remark}[Non-smooth instance]
\label{III:rem:cantor-instance}
The general framework of this section applies without any smoothness, metric,
or manifold hypothesis.  As an illustration, let $\mathcal{C}$ be the
middle-thirds Cantor set with its shift-invariant Cantor measure $\mu_{\mathcal{C}}$
and shift map $S$, as in~\cite{mahon_paper2}.  The cylinder partition
$\mathcal{G}_L = \mathcal{O}_h^{(L)}$ --- the algebra generated by the first $L+1$
delay coordinates --- is an honest refinement scheme: it is increasing and
$\sigma(\bigcup_L \mathcal{G}_L) = \mathcal{B}(\mathcal{C})$ mod $\mu_{\mathcal{C}}$
by the reconstruction theorem.  Each fibre has $\mu_{\mathcal{C}}$-measure
$2^{-(L+1)}$, so the separation defect satisfies $\delta(\mathcal{G}_L) \leq
2^{-(L+1)}$: geometric decay in $L$, with no appeal to Riemannian geometry,
Hölder classes, or mixing rates.  The CE-shadow corollary applies directly ---
a persistent empirical collision floor is evidence against CE --- and the
information horizon $L^* \approx \log_2 N$ marks where the empirical defect
estimate becomes unreliable, for exactly the reason noted in
Remark~\ref{III:rem:info-horizon}.  The smooth dynamical specialisation that
follows is one regime; this is another.
\end{remark}

The remainder of this paper establishes what becomes achievable when the
general framework of this section meets the dynamical structure of~\cite{mahon_paper2}.
Specialisation to a dynamical setting might seem to narrow the question;
instead it strengthens it.  Honest refinement, which must here be a design condition,
becomes free; the separation defect, which is abstract, becomes geometrically
interpretable; and the witnesses, which here are qualitative (floor or no
floor), become quantitative with optimal rates.  That gain is the subject of
Sections~\ref{III:sec:algebra}--\ref{III:sec:conjunction}.
...
\textbf{The measure-preserving system.}\enspace
...
\textbf{The observable and the delay map.}\enspace
...
\textbf{The approximation error.}\enspace
...
\textbf{Fibre structure and collision probability.}\label{III:subsec:fibres}\enspace
...
\begin{remark}[Information horizon]
\label{III:rem:info-horizon}
The fibre structure imposes a hard constraint on what finite data can certify.
At lag~$L$ the observable algebra partitions $X$ into at most $2^{L+1}$
atoms (in the binary-coded case; more generally, the number of fibres grows
exponentially with $L$).
With a sample of size~$n$, the mean occupancy per fibre is $n/2^{L+1}$, which
halves with each additional lag.
The \emph{information horizon} is
\[
  L^* \;:=\; \lfloor \log_2 n \rfloor,
\]
the lag at which mean fibre occupancy drops to~$1$.
Beyond $L^*$ the sample is partitioned across more fibres than it contains
points, so empirical fibre statistics become unreliable.
This gives a first-principles derivation --- from the observable algebra
structure alone --- of why the concentration rate $\varepsilon_n \sim n^{-2s/(2s+d)}$
and the elbow stopping rule of Theorem~\ref{III:thm:conjunction} fire when they do:
doubling the sample size buys exactly one more trustworthy lag.
\end{remark}

\textbf{The data model.}\enspace
...
\textbf{Structural hypotheses.}\enspace
...
\textbf{The empirical witness.}\enspace
...
a sub-$\sigma$-algebra, with $\delta(\mathfrak{m})$ as in~\eqref{III:eq:delta-def}.
...
Bounding $|\mathbbm{1}_S - \mathbbm{1}_{S'}|^2 \leq 1$ in the lemma and taking the supremum
gives $\deltaL \leq \tfrac{1}{2}(\mu\otimes\mu)(\RL)$.

\begin{remark}[Finite analogue of collective exhaustion]
This bound and its converse (Corollary~\ref{III:cor:entropy}, proved
...
genuinely irreducible, as $\sigma$-additivity is proved to be in~\cite{mahon_paper1} --- remains open;
...
\begin{proof}
For an indicator $\mathbbm{1}_S$: the conditional expectation is the $\Lp{2}$-projection, so
$\|\mathbbm{1}_S - \mathbb{E}[\mathbbm{1}_S \mid \mathfrak{m}]\|_{\Lp{2}}^2 \leq \mu(S \triangle E)
\leq \delta(\mathfrak{m})$ for the best $E \in \mathfrak{m}$.
The general bounded case follows by the layer-cake representation
$f = \int_{-M}^{M} \mathbbm{1}_{\{f > t\}}\,dt$ and Minkowski's inequality for integrals,
giving $\|f - \mathbb{E}[f \mid \mathfrak{m}]\|_{\Lp{2}} \leq 2M \cdot \delta(\mathfrak{m})^{1/2}$.
\end{proof}

In the delay setting of Section~\ref{III:sec:setup}, let $\fhat$ denote the empirical risk minimiser over $\AhL$.
...
\end{align}
where $\delta(L) := \delta(\OhL)$ as in~\eqref{III:eq:delta-def} and
...
the set achieving the supremum in~\eqref{III:eq:delta-def} at lag~$L$ and
...
\begin{proof}
Rescale to $y = x/a \in [-1,1]^{L+1}$ and expand $\tilde p$ in the
tensor-product Legendre basis $\{P_\alpha\}_{|\alpha|\leq D}$.
Since $\|P_\alpha\|_{\Lp{\infty}} = 1$, Cauchy--Schwarz gives
$|\tilde p(y)| \leq N(D,L+1)^{1/2}(\sum|c_\alpha|^2)^{1/2}$
with $N(D,L+1) = \binom{D+L+1}{L+1}$.
The Legendre orthogonality $\|P_\alpha\|_{\Lp{2}}^2 = \prod_i(2\alpha_i+1)^{-1}$
gives $\sum|c_\alpha|^2 \leq (2D+1)\|\tilde p\|_{\Lp{2}}^2$ (the
maximum $(2D+1)$ is $L$-independent, achieved at $\alpha=(D,0,\ldots,0)$).
Converting back via
$\|\tilde p\|_{\Lp{2}[-1,1]^{L+1}} = (2a)^{-(L+1)/2}\|p\|_{\Lp{2}(\mathrm{vol})}$
and $\|p\|_{\Lp{2}(\mathrm{vol})} \leq \rho_{\min}^{-1/2}\|p\|_{\Lp{2}(\mu)}$
yields the result.
\end{proof}
...
By the McDiarmid bounded-difference inequality~\cite{McDiarmid1989}:
\[
  P\!\bigl(|\delhat{L}{n} - \mathbb{E}[\delhat{L}{n}]| > t\bigr)
...
\]
By the Ledoux--Talagrand contraction lemma~\cite{LedouxTalagrand1991}, since $f = (p - \cdot)^2$ is
...
\else
\begin{proof}
Uniform boundedness of $\mathcal{F}_{L,D}$ (Lemma~\ref{III:lem:bounded}) gives
$\|f\|_{\Lp{\infty}} \leq 4\MLD^2$ for each $f \in \mathcal{F}_{L,D}$, so a
one-point replacement changes $\delhat{L}{n}$ by at most $4\MLD^2/n$.
The McDiarmid inequality~\cite{McDiarmid1989} gives the stated exponential tail.
The bias term $|\mathbb{E}[\delhat{L}{n}] - \delta(L)|$ is controlled by
symmetrisation and the Ledoux--Talagrand contraction lemma~\cite{LedouxTalagrand1991},
reducing to the Rademacher complexity of $\PD$; since $\PhiL(X)$ is a
$d$-dimensional submanifold in the Takens regime, the VC dimension of $\PD$
is $O(D^d)$~\cite{Gyorfi2002} and $\Rad_n(\PD) \leq C\sqrt{D^d/n}$.
Combining and setting $t = C'n^{-2s/(2s+d)}$ gives the result; the exponent
diverges iff $d > 2s$.
\end{proof}
\fi
...
\else
\begin{proof}
The key input is that the conditional expectation $z \mapsto \mathbb{E}[p \mid \PhiL = z]$
is $\mathrm{H\ddot{o}lder}(\beta)$ uniformly over $p \in \PD$.
This follows from TV-Hölder continuity of the fibre measures:
under conditions~(i)--(ii), a Rokhlin disintegration and implicit-function-theorem
argument give $\|\mu_z - \mu_{z'}\|_{\mathrm{TV}} \leq C_1|z-z'|^\beta$,
and hence $|\mathbb{E}[p|\PhiL=z] - \mathbb{E}[p|\PhiL=z']| \leq \MLD C_1|z-z'|^\beta$
by Lemma~\ref{III:lem:bounded}.
Standard Nadaraya--Watson theory~\cite{Gyorfi2002} for a $\mathrm{H\ddot{o}lder}(\beta)$
regression function on a $\deff(L)$-dimensional domain then gives the stated rate
$n^{-2\beta/(2\beta+\deff(L))}$ at the optimal bandwidth; the $\MLD^2$ factors
cancel in the bias-variance balance and the bound is uniform over $\PD$.
\end{proof}
\fi
...
For the lower bound we use the Sard--Smale theorem~\cite{Smale1965}
applied to two failure maps.
...
\else
\begin{proof}
The upper bound $\deff(L) \leq \min(L+1,d)$ is immediate from dimension counting.
For the lower bound, the Sard--Smale theorem~\cite{Smale1965} is applied to two
failure maps encoding rank deficiency.
In each case, (G1) (distinct orbit points) gives surjectivity of the derivative
of the failure map onto the target, making the failure locus a submanifold of
too low a dimension to carry $\mu$-mass; (G2) places $h$ in the resulting
residual generic set.
Full details of the transversality argument follow the template of
Takens~\cite{Takens1981}.
\end{proof}
\fi
...
functions on a $d$-dimensional domain~\cite{Stone1982}.
...
The collision probability~\eqref{III:eq:collision-prob} defines a natural
...
\end{equation}
This is the \emph{Rényi-$2$ (collision) entropy}~\cite{Renyi1961} of the delay-vector distribution.
...
\begin{proof}
Lemma~\ref{III:lem:cond-var} gives $\deltaL \leq \tfrac{1}{2}(\mu\otimes\mu)(\RL)$.
...
Thus Lemma~\ref{III:lem:balance} guarantees a positive $\nuL$-fraction of
$\varepsilon$-large fibres are $\eta$-balanced without any dynamical hypothesis;
the fibre mixing condition is precisely the upgrade from positive-fraction to
$\nuL$-a.e.\ balance.
...
For deterministic $T$, the conditional regularity kernel specialises to the
Dirac family $\PihL(x,\,\cdot\,) = \delta_{\PhiL(x)}$.
...
\label{III:rem:lyapunov}
The bi-Lipschitz property of $\PhiL$ established in Theorem~\ref{III:thm:dynamics}
under (G1)--(G2)+(US) has a direct bearing on the observable separation exponent
introduced in~\cite{mahon_paper2} (Remark~II.7).
Recall that the \emph{observable separation exponent} is
\[
  \lambda_h(x,y)
  \;:=\; \limsup_{n\to\infty} \frac{1}{n}
  \log d\!\bigl(\PhiL(T^n x),\, \PhiL(T^n y)\bigr),
\]
measuring how fast the observable distinguishes initial conditions under iteration.
The bi-Lipschitz sandwich $c\,d(x,y) \leq d(\PhiL(x),\PhiL(y)) \leq C\,d(x,y)$,
valid $\mu$-a.e.\ under (G1)--(G2)+(US), implies
\[
  \lambda_h(x,y) \;=\; \lambda_1(x,y) \quad \mu\text{-a.e.},
\]
where $\lambda_1$ is the top Lyapunov exponent of $T$ in the ambient metric.
That is, under faithful reconstruction and uniform separation, the observable
separation rate and the geometric instability rate coincide.
This identifies the observable separation exponent as a computable proxy for
$\lambda_1$ accessible directly from time-series data, without knowledge of
the tangent bundle or the derivative cocycle.
\end{remark}

...
The reconstruction theorem states that the observable algebra
$\mathcal{O}_h = \sigma\{h \circ T^n : n \in \mathbb{Z}\}$ generates
...
The probability measure and reconstruction theorem are established
in~\cite{mahon_paper1} and~\cite{mahon_paper2} respectively; this paper
takes both as given and asks what finite data can certify.
...
whether the algebra has captured $\mathcal{B}$, not at what rate the
approximation is converging.
For deterministic $T$, all three witnesses certify the same object,
and finite data suffices to detect it.

...
The reconstruction theorem (\cite{mahon_paper2}) states that the
...
\cite{mahon_paper1} constructs the canonical probability measure
on the observation space; \cite{mahon_paper2} establishes the predictive
kernel and Koopman--Perron duality before proving reconstruction.
...
(all three proved in~\cite{mahon_paper2}).
...
(Definition~\ref{def:fibre-mixing}).
Then
\[
...
For the reverse, Lemma~\ref{lem:hard} with a $\deltaL$-near-maximizer $S^*$

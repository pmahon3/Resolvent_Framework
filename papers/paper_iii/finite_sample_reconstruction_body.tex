...
\begin{abstract}
This paper studies finite-data certification of reconstruction in two settings:
at the level of general query systems, and in the dynamical specialisation.

At the level of a general query system~\cite{mahon_paper1}, finite query
responses certify observational resolution through the \emph{population
separation defect} $\delta(\mathcal{G})$ --- the worst-case symmetric
difference between a target event and its best approximation in a finite
joined query algebra.  Under collective exhaustion and any honest refinement
scheme, $\delta(\mathcal{G}_k) \to 0$.  The empirical collision probability of
the joint response is a U-statistic proxy converging to $\delta(\mathcal{G})$
under IID sampling.  A persistent empirical floor under refinement is evidence
against collective exhaustion: a CE-shadow theorem with no temporal or geometric
assumptions.

In the dynamical specialisation --- a measure-preserving system
$(X,\mathcal{B},\mu,T)$ with observable $h$ and time series
$x_1,\ldots,x_n$ --- honest refinement is automatic and mixing buys
rate-optimal guarantees.  Three theorems constitute the dynamical sharpening.
...
A third computable witness arises from the fibre structure of the delay map.
...
The remainder works in the dynamical specialisation --- a measure-preserving
system $(X, \mathcal{B}, \mu, T)$, observable $h \in L^\infty$, time series
$x_1, \ldots, x_n$ --- where mixing and geometry make rate-optimal guarantees
achievable.  The joined algebra $\mathcal{O}_h^{(L)}$ is an honest refinement
scheme by construction --- not a hypothesis to be verified but a consequence
of the reconstruction theorem itself, which guarantees that $\sigma(\bigcup_L
\mathcal{O}_h^{(L)}) = \mathcal{B}$ modulo $\mu$ once reconstruction holds.
This automaticity is what the dynamical setting secures: in the general query
setting (Section~\ref{III:sec:general}), honest refinement is a design condition
that must be imposed on the query family; here it is free, and the remaining
degrees of freedom --- mixing rate and embedding dimension --- are what
the rate theorems exploit.  The defect specialises to

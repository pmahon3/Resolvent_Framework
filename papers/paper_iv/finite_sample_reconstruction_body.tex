...
$\delta(L)$ as an integral of the within-fibre variance of the near-maximizer
over unseparated trajectory pairs.
This identity isolates the obstruction to equivalence between algebraic and
information-theoretic reconstruction: it fails to reverse when large delay
fibres are monochromatic --- entirely inside or outside the hardest-to-approximate
event.
Under the fibre mixing condition (that $\nu_L$-almost every large fibre is
balanced), the identity yields a third computable witness: the collision entropy
$H_2(\nu_L) := -\log(\mu\otimes\mu)(R_L)$ of the delay-vector distribution,
which diverges if and only if $\delta(L) \to 0$.
The three witnesses --- algebraic, geometric, and information-theoretic ---
all measure the same failure of separation across delay fibres; the entropy
witness is derived from the other two via the fibre balance mechanism.
\end{abstract}
...
The reconstruction theorem (Paper~III,~\cite{mahon_paper3}) states that the
observable algebra $\mathcal{O}_h = \sigma\{h \circ T^n : n \in \mathbb{Z}\}$
generates $\mathcal{B}$ modulo $\mu$ if and only if $h$ is a measure-theoretic
embedding --- an exact condition that says nothing about rates, certificates,
or what can be verified from a finite time series.
Paper~I~\cite{mahon_paper1} constructs the canonical probability measure
on the observation space; Paper~II~\cite{mahon_paper2} establishes
Koopman--Perron duality.
Given a time series $x_1, \ldots, x_n$ from $(X, \mathcal{B}, \mu, T)$
and a single observable $h$, we ask: can one determine from data alone
whether reconstruction holds, at what rate, and with what certificates?
We answer with three theorems: the Algebra Theorem, the Dynamics Theorem,
and the Conjunction Theorem.
...
The elbow rule for $\delhat{L}{n}$ operates upstream of the rate: it asks
``has the algebra captured $\mathcal{B}$?'' rather than ``at what rate is
the approximation converging?''
...
and under (G1)--(G2) on a smooth manifold reaches zero by lag $L_0 \leq d-1$
(all three proved in Paper~III~\cite{mahon_paper3}).
...
genuinely irreducible, as CE is proved to be in Paper~I --- remains open;
see the open problems in Section~\ref{sec:discussion}.
\end{remark}
...
Section~\ref{sec:discussion}), but the general derivability question remains
open; see the open problems below.
This condition is independent of~(US) and is not a standing assumption here;
it is used only in Corollary~\ref{cor:entropy} below.
\end{definition}
...
\label{rem:balance-mechanism}
The bridge theorem (\cite{mahon_bridge}, Theorem~3.3) guarantees that a
positive $\nuL$-fraction of $\varepsilon$-large fibres are $\eta$-balanced
without any dynamical hypothesis.
...
question (Section~\ref{sec:discussion}).
\end{remark}
...
(Definition~\ref{def:fibre-mixing}) is derivable from dynamical hypotheses, or
whether it is an irreducible admissibility condition analogous to CE in Paper~I.

The bridge theorem (\cite{mahon_bridge}, Theorem~3.3) and the analysis in
Remark~\ref{rem:balance-mechanism} settle Step~A: a positive $\nuL$-fraction
of $\varepsilon$-large fibres are $\eta$-balanced, with an explicit lower bound

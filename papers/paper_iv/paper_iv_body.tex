...
% §§1+8 written last: introduction and discussion.
...
The \emph{Dynamics Theorem} shows that under separation-stability, the delay
...
reconstruction and separation-stability.

The key structural insight is a conditional variance identity expressing
$\delta(L)$ as an integral over unseparated trajectory pairs.
This identity yields a third computable witness: the collision entropy
$H_2(\nu_L) := -\log(\mu\otimes\mu)(R_L)$ of the delay-vector distribution,
which diverges if and only if $\delta(L) \to 0$ under a fibre mixing condition.
The three witnesses --- algebraic, geometric, and information-theoretic ---
all measure the same failure of separation across delay fibres.
\end{abstract}
...
This paper is the fourth in a series on observable dynamics.
Paper~I~\cite{mahon_paper1} constructs the canonical probability measure from
an observable algebra via Carathéodory and Stone extension.
Paper~II~\cite{mahon_paper2} establishes Koopman--Perron duality and the
predictive semigroup.
Paper~III~\cite{mahon_paper3} proves the reconstruction theorem: the
observable algebra $\mathcal{O}_h = \sigma\{h \circ T^n : n \in \mathbb{Z}\}$
generates $\mathcal{B}$ modulo $\mu$ if and only if the observable
$h : X \to \mathbb{R}$ is a measure-theoretic embedding.

Paper~IV asks: \emph{what does the reconstruction theorem look like from
finite data?}

Given a time series $x_1, \ldots, x_n$ drawn from a measure-preserving
system $(X, \mathcal{B}, \mu, T)$ and a single observable $h$, can one
determine from data alone whether reconstruction holds?
If so, at what rate?
And with what certificates?

We answer these questions with three theorems: the Algebra Theorem, the
Dynamics Theorem, and the Conjunction Theorem.
The three theorems are calibrated: each is proved under exactly the
hypotheses its proof uses, and the scope of each is named precisely.

\subsection{Three honest theorems}

\textbf{The Algebra Theorem} (Section~\ref{sec:algebra}).
Define the $\sigma$-algebra approximation error
\[ \deltaL = \sup_{S \in \mathcal{B}} \inf_{E \in \OhL} \mu(S \triangle E),\]
which measures how much of $\mathcal{B}$ is not yet captured by the
finite-lag algebra $\OhL = \sigma(h, h\circ T, \ldots, h \circ T^L)$.
This quantity is estimable from data as $\delhat{L}{n}$ (defined
in Section~\ref{sec:setup}).
The elbow stopping rule $\Lhat$ fires when $\delhat{L}{n}$ stops decreasing.
...
This is the main finite-sample rate result.

\textbf{The Dynamics Theorem} (Section~\ref{sec:dynamics}).
The delay map $\PhiL(x) = (h(x), \ldots, h(T^L x))$ encodes the
trajectory of $x$ under the observable.
When $T \in C^r$ and the separation-stability condition~(SS)
holds (infimum of the smallest singular value of $D\PhiL$ over $X$
is bounded away from zero), $\PhiL$ is bi-Lipschitz.
The empirical delay vectors then constitute a second witness: they certify
that points are being separated by the dynamics.
The Dynamics Theorem shows that this kernel witness converges to the true
separation at rate $n^{-\beta/(2\beta+d)}$ and names the places where it can fail.

\textbf{The Conjunction Theorem} (Section~\ref{sec:conjunction}).
The two witnesses measure the same event.
For deterministic $T$, the TV separation $\preddisc{L}(x,x') = 1$ if and
only if $x$ and $x'$ are separated by $\OhL$ --- not merely correlated, but
identical.
This algebraic identity ($\sigma(\PhiL) = \OhL$) is the Markov bridge.
Under reconstruction and~(SS), both witnesses fire correctly and certify the
same object.
The theorem also names two failure modes: reconstruction failure makes the
algebra witness fail; violation of~(SS) makes the kernel witness fail even
when reconstruction holds.

\subsection{What is new}

The $\sigma$-algebra approximation error $\deltaL$ is observable without
knowing the mixing rate, the dimension $d$, or the smoothness $s$.
Standard adaptive methods (cross-validation, Lepski's method) patch the
smoothness oracle but leave the dimension as a residual unknown.
The elbow rule for $\delhat{L}{n}$ patches both simultaneously by operating
upstream of the rate: it asks ``has the algebra captured $\mathcal{B}$?''
rather than ``at what rate is the approximation converging?''

This paper establishes three computable witnesses for reconstruction:
\begin{enumerate}[label=(\roman*)]
  \item $\delhat{L}{n}$ --- the algebraic witness (Section~\ref{sec:algebra});
  \item $\hat{d}_L(x,x')$ --- the delay-map separation witness
    (Section~\ref{sec:dynamics});
  \item $\hat{H}_2(\nuL^{(n)})$ --- the collision entropy witness
    (Section~\ref{sec:algebra}, Corollary~\ref{cor:entropy});
    computationally simpler than (i): no optimisation over sets, only
    delay-vector histogram counts.
\end{enumerate}
Witnesses~(i) and~(iii) are asymptotically equivalent under the fibre mixing
condition (Corollary~\ref{cor:entropy}); witness~(ii) provides the geometric
certificate that the state space is being faithfully separated.
The conditional variance identity (Lemma~\ref{lem:cond-var}, Section~\ref{sec:bias})
expresses the algebraic error $\deltaL$ as an integral over unseparated pairs,
identifying the precise obstruction to equivalence between (i) and (iii) as lack of fibre mixing.

The binary TV separation result (Lemma~\ref{lem:alg-metric}) is specific to
deterministic systems.
For stochastic $T$, the kernel $\PihL(x, \cdot)$ is not a Dirac delta, and
Hölder continuity in TV is both non-trivial and necessary.
This is an open direction.

\subsection{Organisation}

Section~\ref{sec:setup} introduces the objects used throughout, including
fibre structure and collision probability.
Section~\ref{sec:bias} proves the conditional variance identity, the bias bound,
and the easy direction $\deltaL \leq \tfrac{1}{2}(\mu\otimes\mu)(\RL)$.
Section~\ref{sec:concentration} proves concentration of $\delhat{L}{n}$.
Sections~\ref{sec:algebra}, \ref{sec:dynamics}, and~\ref{sec:conjunction}
prove the three main theorems; Section~\ref{sec:algebra} also contains the
entropy characterisation of reconstruction (Corollary~\ref{cor:entropy}).
...
A standalone development appears in \cite{mahon_bridge}.
\end{proof}
...
\begin{proof}
In Lemma~\ref{lem:cond-var}, bound $|\mathbbm{1}_S(x) - \mathbbm{1}_S(x')|^2 \leq 1$
...
\]
This condition is implied by ergodicity of $T$ and is independent of~(SS).
It is not a standing assumption of Paper~IV; it is used only in
Corollary~\ref{cor:entropy} below.
\end{definition}
...
$(\mu \otimes \mu)(\RL)$ introduced in Section~\ref{subsec:fibres}.
\end{corollary}

\begin{proof}
Corollary~\ref{cor:easy} gives $\deltaL \leq \tfrac{1}{2}(\mu\otimes\mu)(\RL)$.
For the reverse, the $(c,\varepsilon)$-mixing condition on a $\deltaL$-near-maximizer
$S^*$ gives $(\mu\otimes\mu)(\RL) \leq \frac{1}{c}\,\deltaL + C(c)\,\varepsilon$
(proved in \cite{mahon_bridge}, Theorem~3.3), so $\deltaL \to 0 \iff
(\mu\otimes\mu)(\RL) \to 0$.
...
Under fibre mixing, Corollary~\ref{cor:entropy} shows that $\Lhat$ could
equivalently be defined as the first $L$ at which $\hat{H}_2(\nuL^{(n)})$
exceeds a threshold calibrated to $n$.
A detailed treatment of $\hat{H}_2$ as a stopping rule appears in
\cite{mahon_bridge}, Section~4.
\end{remark}
...
The Algebra Theorem (Theorem~\ref{thm:algebra}) and the Elbow Location Theorem
(Theorem~\ref{thm:elbow}) require exponential mixing with resolvability constant
$C_\lambda > 4$.
...
Proposition~\ref{prop:concentration} requires $d > 2s$ for the Rademacher
...
work.
This is identified but not proved here.

\textbf{Separation-stability from first principles.}
Condition~(SS) is a hypothesis in Theorems~\ref{thm:dynamics}
and~\ref{thm:conjunction}.
The natural conjecture is that $T$ Anosov with $h$ generic implies~(SS) via
the uniform hyperbolicity constants.
This would close the gap between~(G1)--(G2) and~(SS) in the hyperbolic category.
...
The four-link chain of Proposition~\ref{prop:source2} adapts, but the
Conjunction Theorem's proof structure changes substantially.

\textbf{Sharp rates for the data-driven lag.}
Theorem~\ref{thm:elbow} shows $P(\Lhat = L^*(n)) \to 1$ but does not bound
...
simpler stopping criterion (Remark~\ref{rem:entropy-witness}).
Asymptotic equivalence with $\delhat{L}{n}$ is established under fibre mixing
(Corollary~\ref{cor:entropy}), but a finite-$n$ concentration result for
$\hat{H}_2(\nuL^{(n)})$ --- comparable to Proposition~\ref{prop:concentration}
...
new structural assumptions beyond those of this paper.
This is the most immediate extension of the present work.

\subsection{What is settled}

The four-paper programme is now complete at the level of:
\begin{itemize}
  \item Measure-theoretic foundations (Paper~I).
  \item Koopman--Perron duality and prediction (Paper~II).
  \item Reconstruction theorem and the density bridge (Paper~III).
  \item Finite-sample rates, witnesses, and the honest bridge (Paper~IV).
\end{itemize}
The honest bridge theorem says: algebra reconstruction and kernel separation
are the same event (Lemma~\ref{lem:alg-metric}), both are certifiable from data
(Theorems~\ref{thm:algebra} and~\ref{thm:dynamics}), and their conjunction is
provably reliable when reconstruction and~(SS) hold and provably unreliable when
either fails (Theorem~\ref{thm:conjunction}).
This is not a claim that the two witnesses are informative about everything.
It is a claim that they are informative about each other, and that the Markov
structure of the delay vector --- $\sigma(\PhiL) = \OhL$ --- is why.

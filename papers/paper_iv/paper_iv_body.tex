...
When $T \in C^r$ and the uniform separation condition~(US)
...
Under reconstruction and~(US), both witnesses fire correctly and certify the
same object.
The theorem also names two failure modes: reconstruction failure makes the
algebra witness fail; violation of~(US) makes the kernel witness fail even
...
Corollary~\ref{cor:easy} and its converse (Corollary~\ref{cor:entropy}, proved
under fibre mixing in \S\ref{sec:algebra}) together say: algebraic
...
finite-sample counterpart of collective exhaustion from Paper~I.  There, CE
...
mixing condition plays the same structural role as CE: it is the minimal
...
\]
This condition is implied by ergodicity of $T$ and is independent of~(US).
...
bi-Lipschitz under uniform separation~(US), and the estimated delay
...
(c) The proof of Theorem~\ref{thm:sep-stable} uses~(US) in the lower bound.
...
Uniform separation~(US) ensures that this identity is \emph{empirically
...
Without~(US), the bridge lies: the estimated $\widehat{d}_L$ can diverge
...
Condition~(US) is a hypothesis in Theorems~\ref{thm:dynamics}
...
provably reliable when reconstruction and~(US) hold and provably unreliable when

...
conditional laws.  The resulting quotient defines a minimal
predictive query whose states correspond exactly to distinct
predictive laws.  Prediction is therefore represented by a linear
operator
\[
(K_{Q_*}g)(q_*) = E[g(F)\mid Q_*=q_*]
\]
acting on functions of this predictive state space.  This yields an
observable representation of predictive dynamics expressed entirely in
terms of conditional laws of future observables.
\end{abstract}
...
result of the paper.
Sections~\ref{sec:delay-queries} and~\ref{sec:delay-sufficiency} study
delay-query constructions and the constructive route to predictive
state recovery, with the Takens specialization as a concrete example.
...
\section{Query experiments and the information order}\label{sec:query-experiments}
...
$\Pi_{Q_2}(\pi^{Q_2}_{Q_1}(Q_1),A)=(\Pi_{Q_2}\circ\pi^{Q_2}_{Q_1})(Q_1,A)$.
\end{proof}
...
\section{Delay query chains}\label{sec:delay-queries}

Suppose a time series observable
...
Define delay queries

\[
Q_k(x_t)
=
(x_t,x_{t-\tau},\dots,x_{t-k\tau}).
\]

These form a refinement chain

\[
Q_0\preceq Q_1\preceq Q_2\preceq\cdots.
\]

\begin{lemma}[Monotonic predictive information]
For delay queries
\[
\Pi_{Q_k}\preceq\Pi_{Q_{k+1}}.
\]
\end{lemma}

\bigskip

\section{Delay reconstruction and sufficiency}\label{sec:delay-sufficiency}

\begin{theorem}[Delay sufficiency]
Suppose the delay queries separate predictive equivalence classes,
i.e. whenever two realizations $\omega,\omega'\in\Omega$ induce
distinct predictive laws of the future observable $F$, there exists
some delay depth $k$ such that
\[
Q_k(\omega)\neq Q_k(\omega').
\]

Then there exists a delay depth $k_*$ such that the delay query
\[
Q_{k_*}
\]
is predictively sufficient.
\end{theorem}

This theorem formalizes the emergence of predictive sufficiency from increasing observational resolution.

\begin{remark}[Constructive recovery of the predictive state space]
The delay sufficiency theorem admits a constructive interpretation.
The delay queries
\[
Q_0 \preceq Q_1 \preceq Q_2 \preceq \cdots
\]
form an increasing sequence of observable statistics of the past.
When the delay queries are predictively separating, this sequence
eventually reaches a query that is predictively sufficient.

Consequently sufficiently long delay observations recover the minimal
predictive query
\[
Q_*.
\]
In this sense delay embeddings provide a constructive route to the
predictive state space of the system.
\end{remark}

\begin{remark}[Delay coordinates as predictive statistics]
The delay chain
\[
Q_0\preceq Q_1\preceq Q_2\preceq\cdots
\]
may be viewed as a sequence of increasingly informative statistics of
the observable past.  Delay sufficiency states that, beyond a certain
depth, this sequence captures all predictive information about the
future observable.
\end{remark}

\begin{proposition}[Convergence of delay experiments]
Let
\[
Q_0 \preceq Q_1 \preceq Q_2 \preceq \cdots
\]
be the delay query chain associated with a time series observable.

If the delay queries are predictively separating, then the sequence
of experiments converges in the experiment preorder to the minimal
predictive experiment $Q_*$.
\end{proposition}

\bigskip

\section{Takens specialization}

Let

\begin{itemize}
\item $M$ be a compact manifold of dimension $d$
\item $T:M\to M$ a diffeomorphism
\item $h:M\to\mathbb R$ an observable
\end{itemize}
...
Q_m(x)
=
(h(x),h(T^{-1}x),\dots,h(T^{-(m-1)}x)).
\]

\begin{corollary}[Takens sufficiency]
If
\[
m\ge 2d+1
\]
then the delay observable $Q_m$ separates predictive equivalence
classes and is therefore predictively sufficient.
\end{corollary}

Thus the Koopman operator closes on delay-coordinate observable space.
...
&\downarrow\\
&K_{Q_*}\ (\text{predictive operator})\\
&\downarrow\\
&\text{constructive recovery via delay queries and Takens embedding}
\end{aligned}
\]

...
\begin{theorem}[Predictive state theorem]\label{thm:predictive-state-theorem}
...
\noindent The proof is assembled in Sections~\ref{sec:minimal-predictive}
and~\ref{sec:predictive-sufficiency}: the minimal predictive query
is constructed in Theorem~\ref{thm:minimal-predictive-query};
the factorisation property is Theorem~\ref{thm:predictive-factorization}.
...
\begin{proof}
By the definition of the refinement preorder, $Q_1 \preceq Q_2$ means exactly
that there exists a measurable refinement map $\pi = \pi^{Q_2}_{Q_1}: O_{Q_2}\to O_{Q_1}$
with $Q_1 = \pi\circ Q_2$ on $\Omega$.  Applying this to the law:
$\mathrm{Law}(Q_1) = \pi_\sharp\,\mathrm{Law}(Q_2)$, which is the definition of
$Q_1$'s experiment being a measurable post-processing of $Q_2$'s experiment.
\end{proof}
...
\begin{proof}
Positivity: $g\ge0$ implies $E[g(F)\mid Q=q]\ge0$ pointwise.
Linearity: inherited from linearity of conditional expectation.
Boundedness: $\|K_Q g\|_\infty \le \|g\|_\infty$ since
$|E[g(F)\mid Q=q]|\le E[|g(F)|\mid Q=q]\le\|g\|_\infty$ pointwise.
\end{proof}

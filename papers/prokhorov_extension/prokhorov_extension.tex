...
The Lean~4/Mathlib formalization of this paper has two outstanding
gaps.  First, the directed-set form of the inverse limit theorem
(Proposition~\ref{prop:compact-core}(ii)) is not yet in Mathlib~4.
Second, the notion of a perfect measure
(Definition~\ref{def:sput}) and Musia{\l}'s theorem are absent
from Mathlib; formalizing Corollary~\ref{cor:standard-borel}
would require adding both.  The infrastructure for standard Borel
spaces (\texttt{StandardBorelSpace} typeclass) and inner regularity
(\texttt{Measure.InnerRegular}) is available, providing the
foundation for future formalization.
\end{enumerate}

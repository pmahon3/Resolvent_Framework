...
In classical Prokhorov tightness, one controls mass concentration in
...
is $\sigma$-additive, so that Carath\'eodory's theorem extends it
...
\begin{remark}[When SPUT is achievable]
\label{rem:sput-achievable}
SPUT holds automatically when the refinement maps $\pi^{Q_2}_{Q_1}$
are themselves surjective.  In that case, given any compact
$K_{Q_*}$ at a fine level $Q_*$, one constructs compatible compact
cores by setting $K_Q := \pi^{Q_*}_Q(K_{Q_*})$ for each $Q \preceq
Q_*$; surjectivity of the bonding maps ensures
$\pi^{Q_2}_{Q_1}(K_{Q_2}) = K_{Q_1}$.  Thus the delay query
systems of \emph{Observational Probability from Delay Queries} (where
the refinement maps are coordinate projections, which are surjective)
satisfy SPUT whenever the individual marginals are tight in the
classical sense.
\end{remark}
...
\begin{proof}
(i) The product $\prod_Q K_Q$ is compact by Tychonoff's theorem.
...
K_{Q_1}$ are surjective.  By the \emph{inverse limit theorem} for
...
This is Alexandrov's theorem; it requires only that $K$ is compact
...
One route: if the query system arises from a physical measurement
process with compact outcome spaces (which is natural for bounded
sensors), then the $O_Q$ are automatically compact Hausdorff and
SPUT reduces to the surjectivity of bonding maps on the full spaces
--- which holds whenever the refinement maps are surjective.  Under
these conditions, Theorem~\ref{thm:main} applies with no additional
hypotheses beyond finite compatibility and surjective refinement
maps.  This is the fully observational version: the query system
architecture itself forces $\sigma$-additivity.

\paragraph{Open problems.}

\begin{enumerate}
\item \emph{Surjectivity of refinement maps as the primitive.}
Can the SPUT condition be replaced, in the compact outcome space
setting, by surjectivity of the refinement maps alone?  This would
remove the ``for every $\varepsilon > 0$'' quantifier from the
hypothesis, leaving a purely structural condition.

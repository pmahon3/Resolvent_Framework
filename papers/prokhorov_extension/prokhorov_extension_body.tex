...
argument is standard; it has been formalized in Lean~4 as
\texttt{compactInverseLimit\_nonempty}, using upper-directedness to
supply a common upper bound at each finite stage and proof irrelevance
on the partial-order witnesses to close the coherence conditions.
\end{remark}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[Closed cylinders]{The intermediate theorem: closed cylinders}\label{sec:closed}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The algebraic route, via collective exhaustion
(Chapter~\ref{chap:paper-minus-one}, Theorem~\ref{thm:sp1}), gives
per-level $\sigma$-additivity without any topology.  But it leaves
open the global assembly question: how do the per-level extensions
concentrate mass on $\Omega$?  The present paper's SPUT condition
answers this for Hausdorff outcome spaces, but commits to a topology
assumed as primitive.  A more coherent answer for the program would
derive the topology itself from the observable structure.

Under Stone duality, every Boolean algebra $\mathcal{E}_Q$ of
outcome events determines a canonical compact Hausdorff topology on
its Stone space $S(\mathcal{E}_Q)$, without any external assumption.
If the outcome spaces $O_Q$ are identified with their Stone spaces ---
which is the case precisely when $\mathcal{E}_Q$ separates points of
$O_Q$ and $O_Q$ carries the initial topology from $\mathcal{E}_Q$ ---
then the Hausdorff topology required by this theorem is not assumed
but \emph{derived} from the Boolean algebra of discriminable events.
Whether SPUT can be verified or replaced by an algebraic condition
in this setting is the program's next question for the topological
route.
...
infinite-range observables --- real-valued sensor readings, delay
vectors in $\mathbb{R}^d$, unbounded counts --- it cannot be assumed.
The question is what structural property of the outcome spaces
substitutes for compactness.

\paragraph{The standard Borel route: Musia{\l}'s theorem.}
The existing measure-theoretic literature provides a second
architectural resolution of Stopping Point~2, complementing
Corollary~\ref{cor:architectural} and covering the infinite-range
case.

The key concept is \emph{perfectness} in the sense of Gnedenko and
Kolmogorov~\cite{Bogachev2007}: a probability measure $\mu$ on
$(X, \Bcal)$ is \emph{perfect} if for every measurable $f: X \to
\mathbb{R}$, the image measure $f_*\mu$ is inner regular with respect
to compact sets on $\mathbb{R}$.  Perfectness is a measure-theoretic
regularity condition that does not require topology on $X$ itself.

The connection to the infinite-range case is given by two classical
facts:
\begin{enumerate}[label=(\roman*)]
\item \emph{Standard Borel spaces carry only perfect measures.}
  If $O_Q$ is a standard Borel space --- a measurable space
  isomorphic to a Borel subset of a Polish space --- then every
  Borel probability measure on $O_Q$ is perfect~\cite{Bogachev2007,
  Fremlin2003}.  This follows from the Borel isomorphism theorem
  (any uncountable standard Borel space is measurably isomorphic
  to $\mathbb{R}$) together with inner regularity of finite Borel
  measures on $\mathbb{R}$.
\item \emph{Projective systems of perfect measures have
  $\sigma$-additive limits with no leakage.}
  Musia{\l}~\cite{Musial1980} proved that a projective system of
  perfect probability measures over any directed index set admits a
  unique $\sigma$-additive projective limit $P$ satisfying
  $P(\Omega) = 1$.  No topological conditions on the spaces or maps
  are required beyond measurability.
\end{enumerate}
Combining these facts gives the following corollary, which is a
consequence of~\cite{Musial1980} applied to the query system setting.

\begin{corollary}[Standard Borel architectural extension]
\label{cor:standard-borel}
Let $(\Qcal, \preceq)$ be a countable sequentially upper-directed
query system whose outcome spaces $O_Q$ are standard Borel spaces
and whose refinement maps are measurable.  Then every finitely
compatible family $\{\ell_Q\}$ of Borel probability measures on
the query system extends uniquely to a $\sigma$-additive probability
measure $P$ on $(\Omega, \sigma(\Qcal))$ with $P(\Omega) = 1$.
\end{corollary}

\begin{proof}
Each $\ell_Q$ is a Borel probability measure on a standard Borel
space, hence perfect by fact~(i).  Musia{\l}'s theorem (fact~(ii))
gives a $\sigma$-additive projective limit on $\prod_Q O_Q$ with
full mass on the coherent locus.  The concentration argument of
Section~\ref{sec:concentration} applies: the graph of each
measurable refinement map $\pi^{Q_2}_{Q_1}: O_{Q_2} \to O_{Q_1}$
is measurable in $O_{Q_1} \times O_{Q_2}$ when both spaces are
standard Borel (the diagonal of a standard Borel space is
measurable, since such spaces are $T_2$ and second countable after
upgrading to their Polish topology~\cite{Srivastava1998}).  Each
defect set $D_{Q_1,Q_2}$ is therefore measurable and null, and
the countable union over all comparable pairs is null.
\end{proof}

\paragraph{What the two architectural corollaries together achieve.}
Corollaries~\ref{cor:architectural} and~\ref{cor:standard-borel}
together resolve Stopping Point~2 in the following two architecturally
natural settings:
...
\midrule
Compact outcome spaces & Surjective refinement maps & \ref{cor:architectural} \\
Standard Borel outcome spaces & Measurable refinement maps & \ref{cor:standard-borel} \\
\bottomrule
\end{tabular}
\end{center}

In both cases the conclusion is identical: $\sigma$-additivity is not
an assumption on the observer's laws but a consequence of the query
system's architecture.  The observer need only supply finite
compatibility; the spaces do the rest.

The standard Borel setting covers the main computational case of the
program: delay query systems with outcome spaces $O_{Q_{d,\tau}}
= \mathbb{R}^d$ are standard Borel, so any compatible family of
stationary process marginals extends to a $\sigma$-additive global
measure without any tightness assumption.

\paragraph{The leakage perspective.}
The Yosida--Hewitt theorem~\cite{Dunford1958} provides the
algebraic underpinning for both results.  Every finitely additive
probability measure $\ell$ on a measurable algebra decomposes
uniquely as $\ell = \ell_c + \ell_p$, where $\ell_c$ is
$\sigma$-additive and $\ell_p$ is \emph{purely finitely additive}
--- the component that assigns positive mass to decreasing sequences
of sets intersecting to empty.  The \emph{leakage} of a compatible
family $\{\ell_Q\}$ is
\[
\lambda(\{\ell_Q\}) := 1 - P(\Omega),
\]
the total mass of the purely finitely additive parts that fails to
concentrate on the coherent realization space $\Omega$.

Stopping Point~2 asks when the query system architecture forces
$\lambda = 0$ for every compatible family.  The compact and standard
Borel cases are two settings where this holds.  The open question ---
the remaining form of Stopping Point~2 --- is whether there are
weaker architectural conditions on $(\Qcal, \preceq,
\{\pi^{Q_2}_{Q_1}\})$ that also force $\lambda = 0$, applicable
to spaces that are neither compact nor standard Borel.

\paragraph{Open problems.}

\begin{enumerate}
\item \emph{The topology-free route.}
Both architectural corollaries use some structure on the outcome
spaces: topology (compact Hausdorff) in Corollary~\ref{cor:architectural},
or measurable-space isomorphism to a Polish subspace (standard Borel)
in Corollary~\ref{cor:standard-borel}.  The fully general question
is whether $\lambda = 0$ can be forced from purely order-theoretic
conditions on $(\Qcal, \preceq)$, with no structural assumptions
on the $O_Q$.  The Mallory--Sion theorem~\cite{MallorySion1971}
suggests the obstruction is measurability of the coherent locus
$\Omega$ in the ambient product, and that this measurability is not
guaranteed without additional structure.
...
uncountable $\Qcal$, the correct generalization is a condition on
determining subsystems: if $\Qcal$ admits a countable determining
subsystem $\Qcal_0$ (coherence on $\Qcal_0$ forces coherence on
all of $\Qcal$), the argument applies.  Characterizing when such
subsystems exist is the open problem.

\item \emph{Lean formalization.}
The Lean~4/Mathlib formalization of this paper is nearly complete.
The theorem \texttt{compactInverseLimit\_nonempty}
(Proposition~\ref{prop:compact-core}(ii))
is fully proved via the finite intersection property on
$\prod_Q K_Q$.  The main theorem (\texttt{prokhorov\_extension})
reduces to the algebraic extension theorem
(\texttt{observational\_extension} in \texttt{QuerySystem.lean}),
which packages the Carath\'eodory construction, $\sigma$-subadditivity,
marginal recovery, and uniqueness.  One design point: the Lean
statement of \texttt{prokhorov\_extension} carries an explicit
\texttt{EvalSurjective} hypothesis (surjectivity of $\mathrm{eval}_Q$
onto the full $O_Q$) not present in Theorem~\ref{thm:main}.  This
is because the algebraic route through
\texttt{observational\_extension} requires it directly, whereas the
classical proof derives it from tightness via the inverse limit;
a future refactor should add surjectivity of the bonding maps
$\pi^{Q_2}_{Q_1}$ as a structural field and derive
\texttt{EvalSurjective} from it.  The one remaining \texttt{sorry}
is \texttt{prokhorov\_extension\_polish}
(Corollary~\ref{cor:standard-borel}): the notion of a perfect measure
and Musia{\l}'s theorem are absent from Mathlib~4; formalizing this
corollary would require adding both.  The infrastructure for standard Borel spaces
(the \texttt{StandardBorelSpace} typeclass) and inner regularity
(the \texttt{Measure.InnerRegular} predicate) is available in
Mathlib, providing the foundation for future formalization.
\end{enumerate}
...
\ifcsname thesismode\endcsname\else
\ifcsname thesismode\endcsname\else\input{../acknowledgements}\fi
\fi

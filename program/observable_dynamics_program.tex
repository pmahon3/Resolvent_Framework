...
\title{Discriminative Foundations for Probability and Dynamics: A Program}
...
The goal of the discriminative foundations program is not to replace
...
The discriminative foundations program develops dynamical structure directly
...
Papers~$-1$ through~3 are formalized in Lean~4 / Mathlib.
Papers~0--2 carry zero \texttt{sorry} across
\texttt{QuerySystem.lean}, \texttt{PredictiveState.lean}, and
\texttt{PredictiveOperators.lean}.
Paper~$-1$ (\texttt{DiscriminabilityFoundations.lean}) is complete:
zero sorrys on all main theorems; two infrastructure-blocked sorrys on
\texttt{IsFinitarilyExpressible} and \texttt{ce\_irreducibility}
(ultraproduct construction absent from Mathlib).
Paper~3 (\texttt{ProkhorovExtension.lean}) is nearly complete: one
...
\section{Structure of the discriminative foundations program}

The discriminative foundations program has a central theoretical spine together
...
% Main spine arrows
\draw[arrow] (queries) -- node[right,font=\scriptsize]{Paper 0} (prob);
\draw[arrow] (prob)    -- node[right,font=\scriptsize]{Paper 1} (equiv);
\draw[arrow] (equiv)   --                                       (state);
\draw[arrow] (state)   -- node[right,font=\scriptsize]{Paper 2} (ops);
\draw[arrow] (ops)     --                                       (sg);
\draw[arrow] (sg)      --                                       (gen);
\draw[arrow] (gen)     --                                       (kolm);
...
\caption{Derivational spine of the discriminative foundations program.
...
\subsection{Observational completion \textnormal{(Paper 0)}}
...
\subsection{Predictive state and factorization \textnormal{(Paper 1)}}
...
\subsection{Operator semigroup and duality \textnormal{(Paper 2)}}
...
\subsection{Delay queries and computational instantiation \textnormal{(Paper 3)}}
...
\emph{Resolution: CE is an irreducible primitive (Paper~$-1$, complete).}
The witnessing conjecture --- that sequential upper-directedness alone forces collective
exhaustion --- is \textbf{false}.  The finite-cofinite counterexample on $\mathbb{Q}$
witnesses this: a system satisfying sequential upper-directedness and all structural
hypotheses that fails collective exhaustion at every level.  Formalized as
\texttt{ce\_independence} (zero sorrys) in Lean~4.

More strongly, collective exhaustion is \emph{irreducible}: no condition expressible
in the first-order language of query systems implies it.  Any such condition is
preserved by ultraproducts (L\o s's theorem), and the hyperfilter-based content is
an ultraproduct of point-mass contents that satisfies every structural condition while
failing collective exhaustion.  The Yosida--Hewitt decomposition provides the
general witness: purely finitely additive contents satisfy all algebraic conditions
and are never collectively exhaustive.  Formalized as \texttt{ce\_irreducibility}
(one documented sorry marking the ultraproduct infrastructure gap in Mathlib).

The correct reading is that collective exhaustion is the \emph{minimal irreducible
commitment} separating structural coherence from probability.  The gap between
``coherence'' and ``probability'' in the program's chain is a proved boundary,
not an open conjecture.  CE names exactly what must be assumed and shows it cannot
be smaller.
\end{conjecture}
...
arguments, compactification routes (one-point or Stone--\v{C}ech),
and the Stone duality direction: the Stone space $S(\mathcal{E}_Q)$
of the event algebra is compact Hausdorff canonically, so if SPUT
can be read off from the Stone topology of $\mathcal{E}_Q$ rather
than assumed on $O_Q$, the compact case would subsume the general one
algebraically.
\end{conjecture}

\begin{remark}[SP3: Realizability --- independence established]
Realizability (that the evaluation maps $\mathrm{eval}_Q : \Omega \to O_Q$ are
all surjective) is independent of the other structural hypotheses: sequential
upper-directedness, compatible marginals, and collective exhaustion do not
together force it.  A counterexample is a two-level system with outcome spaces
$\{a,b,c\}$ and $\{x,y\}$ and a non-surjective refinement map missing outcome $c$.

Surjectivity of all refinement maps $\pi^{Q_2}_{Q_1}$ is a \emph{necessary} condition
for realizability (established by the counterexample: the missing outcome $c$ witnesses
the failure).  It is not abstractly sufficient: constructing a coherent inverse-limit
point requires compactness of the outcome spaces (the inverse limit theorem for compact
spaces with surjective bonding maps).  For the programme's concrete systems ---
delay query systems (ambient stream embeddings) and finite-outcome systems (compactness
of finite spaces) --- surjectivity is sufficient and realizability is a structural
consequence.  The compact T2 sufficient condition is formalized in Lean~4
(\texttt{TopologicalQuerySystem.evalSurjective\_of\_compact\_t2}).
The abstract sufficient condition without compactness remains open.

The question of whether realizability follows from almost-sure tightness alone
(without surjectivity of refinement maps) also remains open.
\end{remark}
...
hypothesis of Paper~0, this adjunction becomes an equivalence of
...
\text{(local linear)}.
\]
Papers~0--3 are complete working papers with full proofs, abstracts,
conclusions, related work, and bibliographies; Paper~$-1$ is a complete
self-contained draft.  The Lean formalization covers Papers~$-1$--2
(zero \texttt{sorry} on all main theorems) and Paper~3 (one remaining
\texttt{sorry} in \texttt{prokhorov\_extension\_polish}, Mathlib-blocked).
\paragraph{Immediate priorities.}

\begin{enumerate}

\item \textbf{SP1: CE as proved irreducible primitive (Paper~$-1$).}
The witnessing conjecture (SUD $\Rightarrow$ CE) is \emph{false}:
the finite-cofinite content on $\mathbb{Q}$ is the counterexample
(\texttt{ce\_independence}, zero sorrys).  The stronger irreducibility
result holds: no condition expressible in the first-order language
of query systems implies CE (\texttt{ce\_irreducibility}, one
infrastructure-blocked sorry on ultraproduct construction).
The gap between structural coherence and probability is a proved
boundary, not an open conjecture.  CE is precisely the minimal
irreducible commitment required.  The remaining open question is the
philosophical status of CE as a primitive --- whether it can be
justified from within any formal system is suspected to be undecidable
in character.
...
(Musia\l~1980) outcome spaces.  The remaining open case is locally
compact but non-standard-Borel.  The witnessing-conjecture route to
SP2 is closed (CE is irreducible, not derivable from SUD).  The active
candidate is the Stone duality direction: the Stone space $S(\mathcal{E}_Q)$
of the event algebra is canonically compact Hausdorff, and SPUT may
be algebraically verifiable there, making Hausdorff a structural
consequence rather than a primitive assumption.

\item \textbf{Lean formalization status.}
The SP3 compact T2 case is now fully formalized:
\texttt{TopologicalQuerySystem.evalSurjective\_of\_compact\_t2} (zero sorrys)
proves realizability for compact Hausdorff outcome spaces with continuous
surjective refinement maps, via a Cantor intersection argument over
partial-coherence sets.  One \texttt{sorry} remains in
\texttt{ProkhorovExtension.lean} (\texttt{prokhorov\_extension\_polish}),
blocked on \texttt{PerfectMeasure} and Musia\l's theorem being absent
from Mathlib~4.  No action until Mathlib upstream.
...
formalization across all papers
(\texttt{DiscriminabilityFoundations.lean}, \texttt{QuerySystem.lean},
\texttt{DelayEmbedding.lean}, \texttt{PredictiveState.lean},
\texttt{PredictiveOperators.lean}, \texttt{ProkhorovExtension.lean}).

\subsection*{Fully formalized (zero sorrys on main theorems)}

\begin{itemize}
  \item \emph{Paper~$-1$} (\texttt{DiscriminabilityFoundations.lean}):
        all main results including \texttt{sp1\_iff},
        \texttt{observational\_extension\_of\_collective\_exhaustion},
        \texttt{ce\_independence}, and the SP3 independence and compact
        T2 companion theorems.  Two infrastructure-blocked sorrys remain:
        \texttt{IsFinitarilyExpressible} and \texttt{ce\_irreducibility}
        (require ultraproduct construction for \texttt{QuerySystem},
        absent from Mathlib~4).

  \item \emph{Paper~0} (\texttt{QuerySystem.lean}):
        observational determination theorem (measure and probability
        versions); cylinder family $\pi$-system property; premeasure
        well-definedness, finite additivity, $\sigma$-subadditivity;
        full Carath\'eodory extension; \texttt{observational\_extension}
        and \texttt{observational\_determination}.  Zero sorrys.

  \item \emph{Paper~1} (\texttt{PredictiveState.lean}):
        predictive kernel construction (via Rokhlin disintegration);
        measurability of $\phi_Q$; predictive compatibility; minimal
        predictive query $Q_*$; predictive factorization and sufficiency;
        canonical operator properties.  Zero sorrys.

  \item \emph{Paper~2} (\texttt{PredictiveOperators.lean}):
        semigroup identity and property $K_{t+s}=K_t K_s$ (discrete time);
        Markov constant preservation and positivity; Koopman--Perron
        duality; deterministic specialization; deterministic semigroup
        flow law.  Zero sorrys.
\end{itemize}

\subsection*{Partially formalized}

\begin{itemize}
  \item \emph{Paper~3} (\texttt{ProkhorovExtension.lean}):
        \texttt{compactInverseLimit\_nonempty} and
        \texttt{prokhorov\_extension} proved.  One sorry remains:
        \texttt{prokhorov\_extension\_polish}, blocked on
        \texttt{PerfectMeasure} and Musia\l's theorem being absent
        from Mathlib~4.
\end{itemize}

...
\texttt{PredictiveOperators.lean}).  One analytic obligation remains
open in the formalization: the $\sigma$-subadditivity step in the
Carath\'eodory extension (see Formalization Status section).
...
Each $\nu_Q$ is currently assumed to be countably additive.  The
compression mechanism in the extension theorem suggests a derivation:
start with only finitely additive set functions satisfying normalization
and finite compatibility; show that sequential upper-directedness
forces $\sigma$-additivity as the only globally consistent completion.
This is the deepest available target and the most tractable of the
four stopping points.
\end{conjecture}
...
As of March 2026, the four-paper core spine is established:
\[
(\mathcal Q,\preceq)
\;\xrightarrow{\text{P1}}\;
(\Omega,P)
...
\{K_t\}
\;\xrightarrow{\text{P4}}\;
K_t^N\;\text{(local linear)}.
\]
Papers~1--3 are Lean-verified; Paper~4 is a working note establishing
the delay-query specialization, the probabilistic sufficiency
criterion, and the local linear approximation.

\paragraph{Immediate priorities.}

\begin{enumerate}

\item \textbf{Foundational stopping point SP2.}
Forcing $\sigma$-additivity from finitely additive compatibility under
sequential refinement is the deepest available target and the result
that would most strengthen the program's philosophical claim.  The
compression mechanism in Paper~1's extension proof is the candidate
engine.

...
\emph{Open in the infinite-range setting.}
When the outcome spaces are not compact (e.g.\ real-valued
observables with unbounded range), SPUT is a genuine assumption on
the valuations.  Whether it can be derived from a purely structural
condition on the query system without topological input is the
remaining open form of SP2.
\end{conjecture}
...
\item \textbf{SP2 in the infinite-range setting.}
SP2 is closed for compact outcome spaces (Paper~0, Corollary~6.3).
The remaining open form is: when $O_Q$ are non-compact (e.g.\
real-valued sensors), can $\sigma$-additivity be forced from
structural conditions on the query system alone, without SPUT as an
explicit hypothesis?  This is the topology-free route and the next
foundational target.

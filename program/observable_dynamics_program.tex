...
\emph{Partial progress: Paper~0.}
Each $\nu_Q$ is currently assumed to be countably additive in Paper~1.
Paper~0 (\emph{Finitely Additive Observable Laws and the Prokhorov
Extension for Query Systems}) addresses this directly: starting from
only finitely additive compatible valuations $\{\ell_Q\}$, the
surjective projective uniform tightness (SPUT) condition forces a
unique $\sigma$-additive global extension.  Countable additivity is
derived, not assumed.

The price is topological: Paper~0 requires Hausdorff outcome spaces
and continuous refinement maps.  The fully structural version of SP2
remains open: can $\sigma$-additivity be forced from purely
combinatorial/operational conditions on the query system, without
topological assumptions?  The natural next target is whether SPUT
itself follows from surjectivity of the refinement maps in the compact
outcome space setting --- which would give a fully architecture-driven
derivation.
\end{conjecture}
...
\item \textbf{SP2 next step: SPUT from surjectivity of refinement maps.}
Paper~0 establishes $\sigma$-additivity under SPUT, but SPUT itself is
an assumption (a tightness condition on the family).  The next target
is to show that in the compact outcome space setting, SPUT follows
automatically from surjectivity of the refinement maps.  This would
reduce the hypothesis to a purely architectural condition: the query
system's structure forces $\sigma$-additivity with no additional

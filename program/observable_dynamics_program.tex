...
\texttt{PredictiveOperators.lean}).  Paper~0 has a partial Lean
formalization (\texttt{ProkhorovExtension.lean}); the main open gap
is the directed-set inverse limit theorem with surjective bonding
maps, not yet available in Mathlib~4.  See the Formalization Status
section for details.
...
under finite Boolean operations), show that $\sigma$-closure is forced
by compatibility across refinements.  This would make measurable
structure a theorem of observational compatibility rather than an
input.
\end{conjecture}
...
As of March 2026, the five-paper core spine is established:
\[
\{\ell_Q\}
\;\xrightarrow{\text{P0}}\;
(\Omega,P)
...
\text{(local linear)}.
\]
Paper~0 derives $\sigma$-additivity under SPUT (pen-and-paper, partial
Lean); Papers~1--3 are Lean-verified; Paper~4 is a complete working
paper establishing the delay-query specialization, probabilistic
sufficiency criterion, and local linear approximation.  All five papers
now have proper abstracts, conclusions, related work sections, and
bibliographies.
...
SP2 is now closed for two architectural classes: compact outcome spaces
(Paper~0, Corollary~6.3) and standard Borel outcome spaces
(Musia\l~1980 via perfectness, Paper~0 Discussion, Corollary).
The remaining open form is: when $O_Q$ are locally compact but not
standard Borel, can $\sigma$-additivity be forced from a structural
condition on the query system without SPUT?  Candidate routes include
Mallory--Sion-type arguments and compactification.

\item \textbf{Categorical adjunction (Conjecture~\ref{conj:adjunction}).}
The sketch in the \texttt{categorical-foundations} branch needs to be
promoted to a theorem.  The adjunction $\mathrm{Ext}\dashv\mathrm{Forget}$
would give the program a clean categorical statement and connect it
to the broader literature on categorical probability (Giry monad,
Markov categories).

\item \textbf{Lean formalization of Paper~0.}
The main outstanding gap is the directed-set inverse limit theorem
(Proposition~3.3(ii)).  Once this is available in Mathlib, all steps
of the Paper~0 proof have clear formalization paths.

\item \textbf{Quantitative approximation bounds (Paper~4, Open Question~3).}
The convergence $\|K_t^N - K_t\|_{L^2} \to 0$ is established but
rates are unquantified.  Bounds in terms of mixing rate of $P$,
Fisher--Rao curvature of $O_{Q^*}$, sample size $N$, and
neighbourhood radius $r$ would close the gap between theory and
algorithm design.

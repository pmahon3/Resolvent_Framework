...
satisfies appropriate concentration conditions, their local
commitments assemble into a unique global probability measure $P$
on the realization space $\Omega$.

Chapter~\ref{chap:paper-one} showed that $P$ is entirely determined
by the observable interface.  Nothing is hidden.  The latent space
carries exactly the structure forced by the queries.

Chapter~\ref{chap:paper-two} showed that the predictive content of
each query factors through a minimal sufficient statistic: the
minimal predictive query $Q_*$.  Prediction is structured, not
arbitrary.

Chapter~\ref{chap:paper-three} showed that as time advances, the
...
Chapter~\ref{chap:paper-four} showed that this semigroup can be
